"""
SALT3 residual calculation and model evaluation.

This module provides the core residual calculation machinery for SALT3
model training, including chi-squared computation, regularization terms,
and parameter handling.

Classes
-------
SALTResids
    Main class for computing model residuals and likelihood.

Named Tuples
------------
saltconfiguration
    Model configuration (parameter list, knot locations).
salttrainingresult
    Container for training results (surfaces, parameters, etc.).
"""
...
from saltshaker.training import init_templates
...
def getgaussianfilterdesignmatrix(shape, smoothing):
    ...
    for i in range(window.size // 2 + 1):
        design[i, : window.size // 2 + 1] = gaussian_filter1d(
            1.0 * (np.arange(design.shape[0]) == i), 5
        )[: window.size // 2 + 1]
        design[-i - 1, -(window.size // 2 + 1) :] = gaussian_filter1d(
            1.0 * (np.arange(design.shape[0]) == i), 5
        )[: window.size // 2 + 1][::-1]
    return design
...
class SALTResids:
    """
    Compute residuals and likelihood for SALT3 model fitting.

    This class handles the core computations for SALT3 training: evaluating
    model flux, computing chi-squared residuals against data, applying
    regularization, and computing priors. Uses JAX for GPU acceleration.

    Parameters
    ----------
    datadict : dict
        Dictionary of SALTtrainingSN objects keyed by SNID.
    kcordict : dict
        K-correction dictionary from rdkcor().
    saltconfiguration : saltconfiguration
        Named tuple with parameter list and spline knot locations.
    ...
    phase : ndarray
        Rest-frame phase grid for model evaluation.
    wave : ndarray
        Rest-frame wavelength grid for model evaluation.
    priors : SALTPriors
        Prior probability handler.
    constraints : SALTconstraints
        Parameter constraint handler.
    num_phot : int
        Total number of photometric observations.
    num_spec : int
        Total number of spectroscopic observations.

    Notes
    -----
    The residual vector includes:
    - Photometric residuals (flux - model) / error
    - Spectroscopic residuals (scaled by spec_chi2_scaling)
    - Regularization terms (gradient penalties)
    - Prior terms (Gaussian priors on parameters)
    """
    ...
    def __init__(self, datadict, kcordict, saltconfiguration, options):
        ...
        numrecalparams = [
            (specrecalparams == x).sum() for x in np.unique(specrecalparams)
        ]
        try:
        ...
        self.phaseout = np.linspace(
            self.phaserange[0],
            self.phaserange[1],
            int((self.phaserange[1] - self.phaserange[0]) / self.phaseoutres) + 1,
            True,
        )
        self.interpMethod = "nearest"

        self.wave = np.linspace(
            self.waverange[0],
            self.waverange[1],
            int((self.waverange[1] - self.waverange[0]) / self.waveinterpres) + 1,
            True,
        )
        self.waveout = np.linspace(
            self.waverange[0],
            self.waverange[1],
            int((self.waverange[1] - self.waverange[0]) / self.waveoutres) + 1,
            True,
        )
        ...
        for snid, sn in self.datadict.items():
            if sn.survey not in self.kcordict:
                raise ValueError(
                    f"Could not find corresponding kcor for survey {sn.survey} for SN {snid} "
                )
        ...
        for survey in self.kcordict:
            if survey == "default":
                ...
                self.bbandpbspl = np.interp(
                    self.wave[self.bbandoverlap],
                    self.kcordict["default"]["Bwave"],
                    self.kcordict["default"]["Btp"],
                )
                self.bbandpbspl *= self.wave[self.bbandoverlap]
                self.bbandpbspl /= (
                    np.trapz(self.bbandpbspl, self.wave[self.bbandoverlap]) * HC_ERG_AA
                )
                self.stdmag[survey]["B"] = synphot(
                    self.kcordict[survey]["primarywave"],
                    self.kcordict[survey]["AB"],
                    filtwave=self.kcordict["default"]["Bwave"],
                    filttp=self.kcordict[survey]["Btp"],
                    zpoff=0,
                )
                self.stdmag[survey]["V"] = synphot(
                    ...
                    filttp=self.kcordict[survey]["Vtp"],
                    zpoff=0,
                )
                self.fluxfactor["default"] = {}
                self.fluxfactor[survey]["B"] = 10 ** (
                    0.4 * (self.stdmag[survey]["B"] + 27.5)
                )
                self.fluxfactor[survey]["V"] = 10 ** (
                    0.4 * (self.stdmag[survey]["V"] + 27.5)
                )
                continue
            ...
            for flt in self.kcordict[survey].keys():
                if (
                    ...
                    continue
                if self.kcordict[survey][flt]["magsys"] == "AB":
                    primarykey = "AB"
                elif self.kcordict[survey][flt]["magsys"].upper() == "VEGA":
                    primarykey = "Vega"
                elif self.kcordict[survey][flt]["magsys"] == "BD17":
                    primarykey = "BD17"
                self.stdmag[survey][flt] = (
                    synphot(
                        primarywave,
                        self.kcordict[survey][primarykey],
                        filtwave=self.kcordict[survey][flt]["filtwave"],
                        filttp=self.kcordict[survey][flt]["filttrans"],
                        zpoff=0,
                    )
                    - self.kcordict[survey][flt]["primarymag"]
                )
                self.fluxfactor[survey][flt] = 10 ** (
                    0.4 * (self.stdmag[survey][flt] + 27.5)
                )
                self.kcordict[survey][flt]["minlam"] = np.min(
                    self.kcordict[survey][flt]["filtwave"][
                        self.kcordict[survey][flt]["filttrans"] > 0.01
                    ]
                )
                self.kcordict[survey][flt]["maxlam"] = np.max(
                    self.kcordict[survey][flt]["filtwave"][
                        self.kcordict[survey][flt]["filttrans"] > 0.01
                    ]
                )
        ...
        self.phaseRegularizationBins = np.linspace(
            self.phase[0], self.phase[-1], self.phaseBins[0].size * 2 + 1, True
        )
        self.waveRegularizationBins = np.linspace(
            self.wave[0], self.wave[-1], self.waveBins[0].size * 2 + 1, True
        )

        self.phaseRegularizationPoints = (
            self.phaseRegularizationBins[1:] + self.phaseRegularizationBins[:-1]
        ) / 2
        self.waveRegularizationPoints = (
            self.waveRegularizationBins[1:] + self.waveRegularizationBins[:-1]
        ) / 2
        ...
        (
            self.componentderiv,
            self.dcompdphasederiv,
            self.dcompdwavederiv,
        ...
        try:
            ...
            raise ValueError(
                f"Inconsistent number of color law specifications given {self.colorlaw_function} {self.n_colorpars}"
            )
        self.colorlawfunction = [
            colorlaw.getcolorlaw(function)(npars, self.colorwaverange)
            for function, npars in zip(self.colorlaw_function, self.n_colorpars)
        ]
        ...
        self.relativeregularizationweights = jnp.array(
            ...
            + ([self.mhostregularization] if self.host_component else [])
        )
        ...
        photpadding, efficiency = batching.optimizepaddingsizes(
            self.photometric_zeropadding_batches, photdatasizes
        )

        log.info(
            f"Separating photometric data into {len(photpadding)} batches, at a space efficiency of {efficiency:.0%}"
        )
        efficiencywarningthreshold = 0.3
        if efficiency < efficiencywarningthreshold:
            log.warning(
                f"Efficiency less than {efficiencywarningthreshold:.0%}, consider increasing batching of data"
            )
        specdatasizes = list(getspecdatacounts(datadict))
        if len(specdatasizes):
            specpadding, efficiency = batching.optimizepaddingsizes(
                self.spectroscopic_zeropadding_batches, specdatasizes
            )
            log.info(
                f"Separating spectroscopic data into {len(specpadding)} batches, at a space efficiency of {efficiency:.0%}"
            )
            if efficiency < efficiencywarningthreshold:
                log.warning(
                    f"Efficiency less than {efficiencywarningthreshold:.0%}, consider increasing batching of data"
        ...
        if bool(self.trainingcachefile) and path.exists(self.trainingcachefile):
            log.info(
                f"Loading precomputed quantities from file {self.trainingcachefile}"
            )
            with open(self.trainingcachefile, "rb") as file:
                cachedfile = pickle.load(file)
            ...
            self.allphotdata = sum(
                [[x.photdata[lc] for lc in x.photdata] for x in self.datadict.values()],
                [],
            )
            self.allspecdata = sum(
                [
                    [x.specdata[key] for key in x.specdata]
                    for x in self.datadict.values()
                ],
                [],
            )
            ...
            if sys.stdout.isatty() or in_ipynb:
                iterable = tqdm(iterable, smoothing=0.1)
            self.datadict = {
                snid: (
                    sn
                    if isinstance(sn, SALTfitcacheSN)
                    else SALTfitcacheSN(
                        sn, self, self.kcordict, photpadding, specpadding
                    )
                )
                for snid, sn in iterable
            }
            ...
            if self.trainingcachefile:
                log.info(
                    f"Writing precomputed quantities to file {self.trainingcachefile}"
                )
                    ...
                    pickle.dump(
                        {
                            ...
                            "batchedspecdata": self.batchedspecdata,
                        },
                        file,
                    )
        ...
        self.batchedphotresiduals = batching.batchedmodelfunctions(
            lambda *args, **kwargs: modeledtraininglightcurve.modelresidual(
                *args, **kwargs
            )["residuals"],
        ...
        self.batchedphotlikelihood = batching.batchedmodelfunctions(
            lambda *args, **kwargs: modeledtraininglightcurve.modelloglikelihood(
                *args, **kwargs
            ),
            self.batchedphotdata,
            modeledtraininglightcurve,
        ...
        self.batchedphotvariances = jax.jit(
            batching.batchedmodelfunctions(
        ...
        self.batchedspecresiduals = batching.batchedmodelfunctions(
            lambda *args, **kwargs: modeledtrainingspectrum.modelresidual(
                *args, **kwargs
            )["residuals"],
        ...
        self.batchedspeclikelihood = batching.batchedmodelfunctions(
            lambda *args, **kwargs: modeledtrainingspectrum.modelloglikelihood(
                *args, **kwargs
            ),
            self.batchedspecdata,
            modeledtrainingspectrum,
        ...
        self.batchedspecvariances = jax.jit(
            batching.batchedmodelfunctions(
                modeledtrainingspectrum.modelfluxvariance,
                self.batchedspecdata,
                modeledtrainingspectrum,
            )
        )

        self.batchedspecfluxes = jax.jit(
            batching.batchedmodelfunctions(
                modeledtrainingspectrum.modelflux,
                self.batchedspecdata,
                modeledtrainingspectrum,
            )
        )
        ...
        log.info(
            "Time required to calculate cached quantities {:.1f}s".format(
                time.time() - start
            )
        )
    ...
    def add_model_options(cls, parser, config, addargsonly=False):
        ...
        for param in cls.parameters:
            successful = successful & wrapaddingargument(
                ...
                type=float,
                nargs=3,
                clargformat="--bound_{key}",
                help="bound on %s" % param,
                default=SUPPRESS,
            )
    ...
    def set_param_indices(self):
        ...
        self.errmin = [
            np.min(np.where(self.parlist == f"modelerr_{i}")[0])
            for i in range(self.n_errorsurfaces)
        ]
        self.errmax = [
            np.max(np.where(self.parlist == f"modelerr_{i}")[0])
            for i in range(self.n_errorsurfaces)
        ]
        ...
        self.corrcombinations = [
            x
            for x in combinations_with_replacement(np.arange(self.n_errorsurfaces), 2)
            if x[0] != x[1]
        ]
        if self.host_component:
            self.corrcombinations += [
                (0, "host")
            ]  # =sum([[(i,j) for j in range(i+1,self.n_components)]for i in range(self.n_components)] ,[])

        self.corrmin = tuple(
            [
                np.min(np.where(self.parlist == "modelcorr_{}{}".format(i, j))[0])
                for i, j in self.corrcombinations
            ]
        )
        self.corrmax = tuple(
            [
                np.max(np.where(self.parlist == "modelcorr_{}{}".format(i, j))[0])
                for i, j in self.corrcombinations
            ]
        )
        ...
        for i in range(self.im0.size):
            wavemin += [
                self.waveknotloc[
                    [
                        i % (self.waveknotloc.size - self.bsorder - 1),
                        i % (self.waveknotloc.size - self.bsorder - 1)
                        + self.bsorder
                        + 1,
                    ]
        ...
        self.ix0 = np.array(
            [
                i
                for i, si in enumerate(self.parlist)
                if si.startswith("x0") or si.startswith("specx0")
            ],
            dtype=int,
        )
        self.ix1 = np.array(
            [i for i, si in enumerate(self.parlist) if si.startswith("x1")], dtype=int
        )
        self.ixhost = np.array(
            [i for i, si in enumerate(self.parlist) if si.startswith("xhost")],
            dtype=int,
        )

        self.ic = np.array(
            [
                np.where(np.char.startswith(self.parlist, f"c{i}_"))[0]
                for i in range(len(self.n_colorpars))
            ]
        )
        self.ic0 = np.where(np.char.startswith(self.parlist, "c0_"))[0]
        self.iCL = np.array(
            [
                np.where(self.parlist == f"cl{i}")[0]
                for i in range(len(self.n_colorpars))
            ]
        )
        ...
        self.ispcrcl_norm = np.array(
            [i for i, si in enumerate(self.parlist) if si.startswith("specx0")],
            dtype=int,
        )
        if self.ispcrcl_norm.size == 0:
            self.ispcrcl_norm = np.zeros(self.npar, dtype=bool)
        self.ispcrcl = np.array(
            [i for i, si in enumerate(self.parlist) if si.startswith("spec")], dtype=int
        )  # used to be specrecal
        self.ispcrcl_coeffs = np.array(
            [i for i, si in enumerate(self.parlist) if si.startswith("specrecal")],
            dtype=int,
        )  # used to be specrecal
        if self.ispcrcl.size == 0:
            self.ispcrcl = np.zeros(self.npar, dtype=bool)
        self.imodelerr = np.array(
            [
                i
                for i, si in enumerate(self.parlist)
                if si.startswith("modelerr") or si.startswith("specmodelerr")
            ],
            dtype=int,
        )
        self.imodelerr0 = np.array(
            [i for i, si in enumerate(self.parlist) if si == ("modelerr_0")], dtype=int
        )
        self.imodelerr1 = np.array(
            [i for i, si in enumerate(self.parlist) if si == ("modelerr_1")], dtype=int
        )
        self.imodelerrhost = np.array(
            [i for i, si in enumerate(self.parlist) if si == ("modelerr_host")],
            dtype=int,
        )
        self.imodelcorr = np.array(
            [
                i
                for i, si in enumerate(self.parlist)
                if si.startswith("modelcorr") or si.startswith("specmodelcorr")
            ],
            dtype=int,
        )
        self.imodelcorr01 = np.array(
            [i for i, si in enumerate(self.parlist) if si == ("modelcorr_01")],
            dtype=int,
        )
        self.imodelcorr0host = np.array(
            [i for i, si in enumerate(self.parlist) if si == ("modelcorr_0host")],
            dtype=int,
        )
        ...
        self.icoordinates = np.array(
            [
                np.where(np.char.startswith(self.parlist, f"x{i}"))[0]
                for i in range(1, self.n_components)
            ]
            + list(self.ixhost)
        )
        self.icomponents = [
            np.where(self.parlist == f"m{i}")[0] for i in range(self.n_components)
        ]
        if self.host_component:
            self.icomponents += [self.imhost]
        self.icomponents = np.array(self.icomponents)
        self.iclscat_0, self.iclscat_poly = np.array([], dtype="int"), np.array(
            [], dtype="int"
        )
        if len(self.ispcrcl):
            for i, parname in enumerate(np.unique(self.parlist[self.iclscat])):
                self.iclscat_0 = np.append(
                    self.iclscat_0, np.where(self.parlist == parname)[0][-1]
                )
                self.iclscat_poly = np.append(
                    self.iclscat_poly, np.where(self.parlist == parname)[0][:-1]
                )
        if self.usesurverrfloors:
            self.isurverrfloor = np.where(
                np.char.startswith(self.parlist, "surverrfloor")
            )[0]
        else:
    ...
    def lsqwrap(
        ...
        uncertainties : ndarray
            Parameter uncertainties (for scaling).
        dopriors : bool, optional
            Include prior terms in residuals. Default is True.
        dospecresids : bool, optional
            Include spectroscopic residuals. Default is True.
        usesns : list, optional
            Subset of SNIDs to include. If None, use all.
        suppressregularization : bool, optional
            If True, skip regularization terms. Default is False.
        ...
        """
        residuals = []
        if not (usesns is None):
            raise NotImplementedError("Have not implemented a restricted set of sne")
        lcuncertainties, specuncertainties = uncertainties
        residuals += [
            self.batchedphotresiduals(guess, lcuncertainties, fixuncertainties=True)
        ]

        if dospecresids:
            residuals += [
                self.batchedspecresiduals(
                    guess, specuncertainties, fixuncertainties=True
                )
            ]
            ...
            if self.regularize:
                if suppressregularization:
                    residuals += [
                        ...
                        for func in [
                            self.dyadicRegularization,
                            self.phaseGradientRegularization,
                            self.waveGradientRegularization,
                        ]
                    ]
                else:
                    residuals += [
                        func(guess, self.neff)
                        for func in [
                            self.dyadicRegularization,
                            self.phaseGradientRegularization,
                            self.waveGradientRegularization,
    ...
    def lsqwrap_sources(
        ...
        sources = []
        sources += ["phot"] * numresids(
            lambda x: self.batchedphotresiduals(
                x, uncertainties[0], fixuncertainties=True
            )
        )
        if dospecresids:
            sources += [f"spec"] * numresids(
                lambda x: self.batchedspecresiduals(
                    x, uncertainties[1], fixuncertainties=True
                )
            )
        ...
        if dopriors:
            ...
            for label, func in [
                ("reg_dyad", self.dyadicRegularization),
                ("reg_phase", self.phaseGradientRegularization),
                ("reg_wave", self.waveGradientRegularization),
            ]:
    ...
    def getChi2Contributions(self, X, **kwargs):
        ...
        assert np.isin(
            sources, ["reg_dyad", "reg_phase", "reg_wave", "phot", "spec", "priors"]
        ).all()
        ...
        def loop():
            for name, abbrev in [
                ...
                ("Spectroscopic", "spec"),
                ("Prior", "priors"),
                ("Dyadic Regularization", "reg_dyad"),
                ("Phase Regularization", "reg_phase"),
                ("Wave Regularization", "reg_wave"),
            ]:
    ...
    def maxlikefit(
        ...
        if cachedresults is None:
            cachedresults = None, None
        if not (usesns is None):
            raise NotImplementedError("Have not implemented a restricted set of sne")

        loglike = self.batchedphotlikelihood(
            guess, cachedresults[0], fixuncertainties, fixfluxes
        )

        if dospec:

            loglike += self.batchedspeclikelihood(
                guess, cachedresults[1], fixuncertainties, fixfluxes
            )
        ...
        if dopriors:
            ...
            if self.regularize:
                loglike += sum(
                    [
                        -(func(guess, self.neff) ** 2).sum() / 2.0
                        for func in [
                            self.dyadicRegularization,
                            self.phaseGradientRegularization,
                            self.waveGradientRegularization,
    ...
    def estimateparametererrorsfromhessian(self, X, hessian=None):
        ...
        if hessian is None:
            ...
            logging.debug(
                "Allowing parameters {np.unique(self.parlist[varyingParams])} in calculation of inverse Hessian"
            )
            X = jnp.array(X)
            lsqwrap_partial = lambda x: self.lsqwrap(
                X.at[varyingParams].set(x),
                self.calculatecachedvals(X, target="variances"),
                suppressregularization=True,
                dospecresids=self.dospec,
            )
        ...
        sigma = (
            linalg.cho_solve(
                linalg.cho_factor(hessian / preconditioningelementwise),
                np.identity(scales.size),
            )
            / preconditioningelementwise
        )
    ...
    def computeuncertaintiesfromparametererrors(self, X, sigma, smoothingfactor=150):
        ...
        if len(iOK):
            ...
            sigma[
                np.where(~varyingParams)[0][iOK], np.where(~varyingParams)[0][iOK]
            ] = 100
        ...
        corrcombinations = [
            ...
            for x in combinations_with_replacement(
                np.arange(self.n_components + self.host_component), 2
            )
        ]
        surfaces = {
            x: np.empty((self.phaseout.size, self.waveout.size))
            for x in corrcombinations
        }
        ...
        for chunkindex in iterable:
            ...
            spline_derivs = np.empty(
                [
                    self.phaseout.size,
                    min(self.waveout.size - chunkindex, chunksize),
                    self.im0.size,
                ]
            )
            for i in range(self.im0.size):
            ...
            if smoothingfactor > 0:
                smoothingmatrix = getgaussianfilterdesignmatrix(
                    spline2d.shape[0], smoothingfactor / self.waveoutres
                )
        ...
        stderrs = [
            np.clip(np.sqrt(surfaces[(i, i)]), 0, np.abs(comp).max() * 2)
            for i, comp in zip(
                range(self.n_components + self.host_component), (components)
            )
        ]
        correlations = [
            (
                ...
                j,
                np.clip(
                    np.nan_to_num(surfaces[(i, j)] / (stderrs[i] * stderrs[j])), -1, 1
                )
                * (stderrs[i] * stderrs[j]),
            )
            for i, j in corrcombinations
    ...
    def processoptimizedparametersforoutput(
        self, rescaledparams, rawparams, parametercovariance=None
    ):
        """
        Convert optimized parameters to output model surfaces.

        Transforms the raw spline coefficients into evaluated M0, M1,
        error surfaces, color law, and per-SN parameters suitable for
        output files.

        Parameters
        ----------
        rescaledparams : ndarray
            Parameters after rescaling/transformation.
        rawparams : ndarray
            Raw optimizer parameters.
        parametercovariance : ndarray, optional
            Covariance matrix for uncertainty propagation.

        Returns
        -------
        salttrainingresult
            Named tuple containing model surfaces and metadata.
        """
        X = np.array(rawparams)
        Xfinal = np.array(rescaledparams)
        errmodel = self.ErrModel(
            Xfinal, evaluatePhase=self.phaseout, evaluateWave=self.waveout
        )
        componentnames = ["M" + str(i) for i in range(self.n_components)] + (
            ["Mhost"] if self.host_component else []
        )

        if parametercovariance is None:
            dataerrs, datacovs = None, None
        else:
            dataerrs, datacovs = self.computeuncertaintiesfromparametererrors(
                X, parametercovariance
            )

        covmodel = []
        corrmodel = self.CorrelationModel(
            Xfinal, evaluatePhase=self.phaseout, evaluateWave=self.waveout
        )
        for i, combination in enumerate(self.corrcombinations):
            ...
            covmodel += [
                (
                    *combination,
                    corrmodel[i]
                    * errmodel[combination[0]]
                    * errmodel[-1 if combination[1] == "host" else combination[1]],
                )
            ]

        return salttrainingresult(
            num_lightcurves=self.num_lc,
            num_spectra=self.num_spectra,
            num_sne=len(self.datadict),
            parlist=self.parlist,
            params=Xfinal,
            params_raw=rawparams,
            phase=self.phaseout,
            wave=self.waveout,
            componentnames=componentnames,
            componentsurfaces=self.SALTModel(
                Xfinal, evaluatePhase=self.phaseout, evaluateWave=self.waveout
            ),
            ...
            clpars=Xfinal[self.iCL],
            clscat=self.colorscatter(Xfinal, self.waveout),
            snparams={
                sn: {
                    (
                        name
                        if not ((name == "c0") and (len(self.n_colorpars) == 1))
                        else "c"
                    ): X[self.parlist == "_".join([name, sn])][0]
                    for name in (
    ...
    def SALTModel(self, x, evaluatePhase=None, evaluateWave=None):
        ...
        for i in range(self.n_components + self.host_component):
            ...
            if self.bsorder != 0:
                ...
                n_repeat_phase_extra = -1 * (
                    n_repeat_phase * (self.phaseknotloc.size - 1) % phase.size
                )
                if n_repeat_phase_extra == 0:
                    n_repeat_phase_extra = None
                n_repeat_wave = int(wave.size / (self.waveknotloc.size - 1)) + 1
                n_repeat_wave_extra = -1 * (
                    n_repeat_wave * (self.waveknotloc.size - 1) % wave.size
                )
                if n_repeat_wave_extra == 0:
                    n_repeat_wave_extra = None
                surface = np.repeat(
                    np.repeat(
                        comppars.reshape(
                            [self.phaseknotloc.size - 1, self.waveknotloc.size - 1]
                        ),
                        n_repeat_phase,
                        axis=0,
                    ),
                    n_repeat_wave,
                    axis=1,
                )[:n_repeat_phase_extra, :n_repeat_wave_extra]
            components += [surface]
    ...
    def CorrelationModel(self, x, evaluatePhase=None, evaluateWave=None):
        ...
        for min, max in zip(self.corrmin, self.corrmax):
            ...
            if self.errbsorder == 0:
                binphasecenter = (
                    (self.errphaseknotloc)[1:] + (self.errphaseknotloc)[:-1]
                ) / 2
                binwavecenter = (
                    (self.errwaveknotloc)[1:] + (self.errwaveknotloc)[:-1]
                ) / 2
                interp = RegularGridInterpolator(
                    (binphasecenter, binwavecenter),
                    errpars.reshape(binphasecenter.size, binwavecenter.size),
                    "nearest",
                    False,
                    0,
                )
    ...
    def updateEffectivePoints(self):
        ...
        """
        # Clean out array
        self.neffRaw = np.zeros(
            (self.phaseRegularizationPoints.size, self.waveRegularizationPoints.size)
        )
        ...
        for snid, sn in self.datadict.items():
            for k, spectrum in sn.specdata.items():
                ...
                if phase < self.phaseRegularizationBins[0]:
                    ...
                    phaseIndex = np.where(
                        (phase >= self.phaseRegularizationBins[:-1])
                        & (phase < self.phaseRegularizationBins[1:])
                    )[0][0]
                ...
                self.neffRaw[phaseIndex, :] += ss.binned_statistic(
                    restWave,
                    spectrum.flux / spectrum.flux.max() / len(spectrum),
                    bins=self.waveRegularizationBins,
                    statistic="sum",
                ).statistic
        ...
        if not np.any(np.isinf(self.neff)):
            log.warning(
                "Regularization is being applied to the entire phase/wavelength space: consider lowering neffmax (currently {:.2e})".format(
                    self.neffmax
                )
            )

        self.neff = np.clip(self.neff, self.nefffloor, None).flatten()
        self.suppressedneff = np.select(
            [self.neff >= self.neffmax, self.neff < self.neffmax],
            [np.tile(10, self.neff.shape), np.tile(10, self.neff.shape)],
    ...
    def plotEffectivePoints(self, phases=None, output=None):
        ...
        if phases is None:
            ...
            plt.xticks(
                xticks,
                [
                    "{:.0f}".format(self.waveRegularizationPoints[int(x)])
                    for x in xticks
                ],
            )
            ...
            plt.yticks(
                yticks,
                [
                    "{:.0f}".format(self.phaseRegularizationPoints[int(x)])
                    for x in yticks
                ],
            )
            ...
            for i in inds:
                plt.plot(
                    self.waveRegularizationPoints[:],
                    self.neffRaw[i, :],
                    label="{:.1f} days".format(self.phaseRegularizationPoints[i]),
                )
            plt.ylabel("$N_eff$")
            plt.xlabel("$\lambda (\AA)$")
            plt.xlim(
                self.waveRegularizationPoints.min(), self.waveRegularizationPoints.max()
            )
            plt.legend()
    ...
    def dyadicRegularization(self, x, neff):
        """
        Compute dyadic (cross-derivative) regularization penalty.

        Penalizes non-separability in the model surface, i.e., departures
        from flux(phase, wave) = g(phase) * h(wave). This encourages
        smooth, physically motivated spectral evolution.
        ...
        x : ndarray
            Current parameter values.
        neff : ndarray
            Effective number of observations at each grid point.

        Returns
        -------
        ndarray
            Regularization residuals for dyadic term.
        """
        ...
        if self.regularizationScaleMethod == "clippedflux":
            denominator = jnp.clip(jnp.abs(fluxes), 0.004, None)
        elif self.regularizationScaleMethod == "fixed":
            denominator = 1
        else:
            raise ValueError(
                "Unknown regularization scale method %s"
                % self.regularizationScaleMethod
            )
        ...
        normalization = jnp.sqrt(
            self.regulardyad
            * self.relativeregularizationweights
            / ((self.waveBins[0].size - 1) * (self.phaseBins[0].size - 1)) ** 2.0
        )
        # 0 if model is locally separable in phase and wavelength i.e. flux=g(phase)* h(wavelength) for arbitrary functions g and h
        numerator = (
            dfluxdphase * dfluxdwave - d2fluxdphasedwave * fluxes
        ) / denominator**2
        return (
            normalization[np.newaxis, :] * (numerator / (neff[:, np.newaxis]))
        ).flatten()

    def phaseGradientRegularization(self, x, neff):
        """
        Compute phase gradient regularization penalty.

        Penalizes rapid variations in the model surface along the phase
        axis, weighted by the effective number of observations.
        ...
        -------
        ndarray
            Regularization residuals for phase gradients.
        """
        coeffs = x[self.icomponents]
        fluxes = self.componentderiv @ coeffs.T
        if self.regularizationScaleMethod == "clippedflux":
        ...
        dfluxdphase = self.dcompdphasederiv @ coeffs.T
        normalization = jnp.sqrt(
            self.regulargradientphase
            * self.relativeregularizationweights
            / ((self.waveBins[0].size - 1) * (self.phaseBins[0].size - 1))
        )
        return (
            normalization[np.newaxis, :]
            * (dfluxdphase / denominator / neff[:, np.newaxis])
        ).flatten()

    def waveGradientRegularization(self, x, neff):
        """
        Compute wavelength gradient regularization penalty.

        Penalizes rapid variations in the model surface along the wavelength
        axis, weighted by the effective number of observations.
        ...
        -------
        ndarray
            Regularization residuals for wavelength gradients.
        """
        coeffs = x[self.icomponents]
        fluxes = self.componentderiv @ coeffs.T
        if self.regularizationScaleMethod == "clippedflux":
        ...
        dfluxdwave = (
            ...
            * (
                (self.waveRegularizationPoints / self.waveRegularizationPoints.max())
                ** self.waveregularizationreweighting
            )[np.newaxis, :, np.newaxis]
        ).reshape(
            self.phaseRegularizationPoints.size * self.waveRegularizationPoints.size, 2
        )
        return (
            normalization[np.newaxis, :]
            * (dfluxdwave / denominator / neff[:, np.newaxis])
        ).flatten()
...
def fourierRegularization(self, x, neff):
    ...
    for i, surface in enumerate(surfaces):
        ...
        smoothness_profile = jnp.exp(
            -0.5 * (jnp.log(wave_centers / optical_center) / log_width) ** 2
        )
        ...
        for j in range(surface.shape[1]):
            ...
            scale_factor = (
                1.0 / smoothness_profile[j]
            )  # Invert so higher values = more smoothing
            ...
            for k in range(surface.shape[0]):
                ...
                penalty = penalty.at[k, :].set(
                    penalty[k, :] + wave_mask_freq * wave_penalty
                )

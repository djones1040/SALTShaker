...
class modeledtraininglightcurve(modeledtrainingdata):
    ...
    def __init__(self, sn, lc, residsobj, kcordict, padding=0):
        ...
        self.imodelerrs = np.array(
            [np.arange(x, y + 1) for x, y in (zip(residsobj.errmin, residsobj.errmax))]
        )
        ...
        for i in np.where(isrelevant)[0]:
            derivInterp[:, :, i] = bisplev(
                ...
                (
                    ...
                    residsobj.waveknotloc,
                    np.arange(residsobj.im0.size) == i,
                    residsobj.bsorder,
                    residsobj.bsorder,
                ),
            )
        ...
        if residsobj.errbsorder == 0:
            ...
            for i in np.where(isrelevant)[0]:
                derivInterp[:, i] = bisplev(
                    ...
                    self.lambdaeffrest,
                    (
                        residsobj.errphaseknotloc,
                        residsobj.errwaveknotloc,
                        np.arange(residsobj.imodelerr0.size) == i,
                        residsobj.errbsorder,
                        residsobj.errbsorder,
                    ),
                )
            self.errordesignmat = sparse.BCOO.fromdense(
                np.concatenate(
                    (derivInterp, np.zeros((padding, residsobj.imodelerr0.size)))
                )
            )
...
class modeledtrainingspectrum(modeledtrainingdata):
    ...
    def __init__(self, sn, spectrum, k, residsobj, padding=0):
        ...
        self.recaltermderivs = np.concatenate(
            (self.recaltermderivs, np.zeros((padding, pow.size)))
        )
        ...
        self.varianceprefactor = np.concatenate(
            (self.varianceprefactor, np.zeros(padding))
        )
        if residsobj.errbsorder == 0:
            errorwaveind = (
        ...
        self.imodelerrs = np.array(
            [
                (np.where(residsobj.parlist == f"specmodelerr_{i}")[0])
                for i in range(residsobj.n_errorsurfaces)
            ]
        )
        self.imodelcorrs = np.array(
            [
                (np.where(residsobj.parlist == "specmodelcorr_{}{}".format(i, j))[0])
    ...
    def modelfluxvariance(self, pars):
        ...
        for (i, j), correlation in zip(self.imodelcorrs_coordinds, pars.modelcorrs):
            errorsurfaces = (
                errorsurfaces
                + 2 * correlation * coordinates[i] * coordinates[j] * errs[i] * errs[j]
            )
        errorsurfaces = self.errordesignmat @ errorsurfaces

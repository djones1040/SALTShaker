"""
Base classes for SALT3 training.

This module provides the base class infrastructure for SALT3 model training,
including configuration handling, data cuts, and survey definitions.

Classes
-------
SNCut
    Defines a single cut criterion for supernova selection.
TrainSALTBase
    Base class providing configuration and data handling for training.
"""
import os, sys
import argparse
import configparser
...
class SNCut:
    ...
    ----------
    description : str
        Human-readable description of the cut (e.g., "epochs near peak").
    requirement : float
        Minimum value required to pass the cut.
    valfunction : callable
        Function that takes a supernova object and returns a numeric value
        to compare against the requirement.

    Examples
    --------
    >>> cut = SNCut('epochs near peak', 2, lambda sn: len(sn.photdata))
    >>> cut.passescut(some_sn)
    True
    """
    ...
    def cutvalue(self, sn):
        """Return the cut metric value for a given supernova."""
        return self.__valfunction__(sn)

    def passescut(self, sn):
        """Return True if the supernova passes this cut."""
        return self.cutvalue(sn) >= self.requirement


class TrainSALTBase:
    """
    Base class for SALT3 model training.

    Provides configuration parsing, command-line argument handling,
    survey definitions, and data quality cuts. This class is inherited
    by TrainSALT which implements the actual training logic.
    ...
    surveylist : list of str
        List of survey names defined in the configuration.
    options : argparse.Namespace
        Parsed configuration options.
    kcordict : dict
        K-correction dictionary (set after initialization).
    """
    ...
    def add_user_options(self, parser=None, usage=None, config=None):
        """
        Add I/O and general user options to argument parser.

        Adds options for input/output files, data cuts, survey definitions,
        and general training flags. Model and optimizer options are added
        separately by their respective classes.

        Parameters
        ----------
        parser : argparse.ArgumentParser, optional
            Existing parser to add arguments to. If None, creates new parser.
        usage : str, optional
            Usage string for the parser.
        config : configparser.ConfigParser, optional
            Configuration file parser with default values.

        Returns
        -------
        argparse.ArgumentParser
            Parser with I/O and user options added.
        """
        if parser == None:
            parser = ConfigWithCommandLineOverrideParser(
                usage=usage, conflict_handler="resolve", add_help=False
            )
        ...
        parser.add_argument(
            "--clobber", default=False, action="store_true", help="clobber"
        )
        parser.add_argument(
            "configpositional",
            nargs="?",
        ...
        successful = successful & wrapaddingargument(
            ...
            type=str,
            action=FullPaths,
            help="Configuration file providing hyperparameters to the optimizer",
            default=None,
        )
        ...
        successful = successful & wrapaddingargument(
            ...
            "spectra_cut",
            type=float,
            help="Makes cut so that only spectra of certain S/N are used ",
            default=0,
        )

        msg = "range of obs filter central wavelength (A)"
        successful = successful & wrapaddingargument(
            config, "iodata", "filtercen_obs_waverange", type=float, nargs=2, help=msg
        )
        ...
        # survey definitions
        self.surveylist = [
            section.replace("survey_", "")
            for section in config.sections()
            if section.startswith("survey_")
        ]
    ...
    def snshouldbecut(self, sn, cuts):
        ...
        if not all(passescuts):  # or iPreMaxCut < 2 or medSNR < 10:
            ...
            log.debug(
                ", ".join(
                    [
                        f"{cut.cutvalue(sn)} {cut.description} ({cut.requirement} needed)"
                        for cut in cuts
                    ]
                )
            )
    ...
    def checkFilterMass(self, z, survey, flt):
        ...
        try:
            ...
            raise RuntimeError(
                f"filter {flt} not found in kcor file for survey {survey}.  Check your config file"
            )
        try:
            filttrans = self.kcordict[survey][flt]["filttrans"]
        except:
            raise RuntimeError(
                "filter %s not found in kcor file for SN %s" % (flt, sn)
            )
        ...
        return (
            np.trapz(
                (filttrans * filtwave / (1 + z))[filtRange],
                filtwave[filtRange] / (1 + z),
            )
            / np.trapz(filttrans * filtwave / (1 + z), filtwave / (1 + z))
            > 1 - self.options.filter_mass_tolerance
        )

    def getcuts(self):
        """
        Return list of SNCut objects defining the sample selection.

        Returns
        -------
        list of SNCut
            Cut objects for epochs, phase coverage, filters, and optionally spectra.
        """
        ...
        def checkfitprob(sn):
            ...
            if hasvalidfitprob:
                ...
                log.warning(
                    f"SN {sn.snid} does not have a valid salt2 fitprob, including in sample"
                )
                return 1
    ...
    def mkcuts(self, datadict):
        """
            Apply quality cuts to the training sample.

            Filters supernovae and their data based on phase range, wavelength
            coverage, epoch requirements, and other quality criteria.
        ...
        Dictionary of training data keyed by SN ID.
            ...
            -------
            outdict : dict
        SNe that pass all cuts.
            cutdict : dict
        SNe that were cut from the sample.

            Notes
            -----
            Default cuts require:
            - 4 epochs at -10 < phase < 35 days
            - 1 measurement near peak (-10 < phase < 5)
            - 1 measurement post-peak (5 < phase < 20)
            - 3 filters with coverage near peak
            - SALT2 fitprob > fitprobmin (if available)
        """
        # Define cuts
        cuts = self.getcuts()
        ...
        sumattr = (
            lambda x, sndict: len(sndict)
            if x == "num_sn"
            else sum([getattr(sndict[snid], x) for snid in sndict])
        )
        descriptionandattrs = [
            ("photometric observations", "num_photobs"),
            ("spectroscopic observations", "num_specobs"),
            ("light-curves", "num_lc"),
            ("spectra", "num_spec"),
        ]
        ...
        for snid in datadict:
            ...
            for k in list(specdata.keys()):
                ...
                spectrum = specdata[k]
                if (
                    spectrum.phase < self.options.phaserange[0]
                    or spectrum.phase > self.options.phaserange[1] - 3
                ):
                    specdata.pop(k)
                    continue
                # remove spectral data outside wavelength range
                inwaverange = (
                    spectrum.wavelength > (self.options.waverange[0] * (1 + z))
                ) & (spectrum.wavelength < (self.options.waverange[1] * (1 + z)))
                ...
                if len(clippedspectrum):
                    ...
                    continue
                if (
                    np.median(spectrum.flux / spectrum.fluxerr)
                    < self.options.spectra_cut
                ):
                    specdata.pop(k)
                    continue
            ...
            for flt in sn.filt:
                ...
                if not self.filter_select(sn.survey, flt):  # RK Nov 7 2022
                    ...
                    inphaserange = (
                        lightcurve.phase > self.options.phaserange[0]
                    ) & (lightcurve.phase < self.options.phaserange[1])
        ...
        for attr, desc, initial, final, cut in zip(
            attrs, descriptions, initialdemos, finaldemos, sncutdemos
        ):
            log.info(
                f"{initial-(final+cut)} {desc} removed as a result of cuts for phase and wavelength range"
            )

        log.info(
            f"{len(datadict)} SNe initially, {len(cutdict)} SNe cut from the sample"
        )
        ...
        log.info(
            ...
            + " remaining after all cuts"
        )
        log.info(f"Unique filters: {np.unique(filters)}")
        return outdict, cutdict

    def filter_select(self, survey, flt):
        if flt in self.options.__dict__[
            f"{survey.split('(')[0]}_ignore_filters"
        ].replace(" ", "").split(","):
            return False
        else:
            lambdaeff = self.kcordict[survey][flt]["lambdaeff"]
            if (
                lambdaeff < self.options.filtercen_obs_waverange[0]
                or lambdaeff > self.options.filtercen_obs_waverange[1]
            ):
                return False
        return True

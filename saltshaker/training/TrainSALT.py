...
"""
SALT3 model training module.

This module provides the main training infrastructure for the SALT3
spectrophotometric model of Type Ia supernovae. It includes parameter
initialization, model fitting via Gauss-Newton optimization, output
generation, and validation routines.

Classes
-------
TrainSALT
    Main training class that handles model fitting and output.
RunTraining
    Entry point class that parses configuration and runs training.

Examples
--------
Command line usage::

    trainsalt -c training.conf

Programmatic usage::

    from saltshaker.training.TrainSALT import RunTraining
    runner = RunTraining()
    runner.main(configfile='training.conf')

See Also
--------
saltshaker.training.base : Base class and utilities for training.
saltshaker.training.saltresids : Residual calculation for fitting.
saltshaker.training.optimizers : Optimization algorithms.
"""
...
from saltshaker.training.init_templates import (
    init_hsiao,
    init_kaepora,
    init_errs,
    init_errs_percent,
...
def _RatioToSatisfyDefinitions(phase, wave, kcordict, components):
    """Ensures that the definitions of M1,M0,x0,x1 are satisfied."""

    Bmag = synphot(
        kcordict["default"]["primarywave"],
        kcordict["default"]["AB"],
        filtwave=kcordict["default"]["Bwave"],
        filttp=kcordict["default"]["Btp"],
        zpoff=0,
    )
    ...
    m0Bflux = (
        np.sum(kcordict["default"]["Bpbspl"] * int1d([0]), axis=1)
        * (wave[1] - wave[0])
        * Bflux
    )
...
def _specflux(
    obsphase, obswave, m0phase, m0wave, m0flux, m1flux, colorlaw, z, x0, x1, c, mwebv
):

    modelflux = x0 * (m0flux + x1 * m1flux) * 1e-12 / (1 + z)

    m0interp = interp1d(
        np.unique(m0phase) * (1 + z),
        m0flux * 1e-12 / (1 + z),
        axis=0,
        kind="nearest",
        bounds_error=False,
    ...
    mwextcurve = 10 ** (
        -0.4 * extinction.fitzpatrick99(obswave.astype(float), mwebv * 3.1)
    )
...
class TrainSALT(TrainSALTBase):
    """
    Main SALT3 model training class.

    Handles the complete training workflow including parameter initialization,
    model fitting, bootstrap error estimation, output file generation, and
    validation. Inherits configuration handling from TrainSALTBase.

    Attributes
    ----------
    options : argparse.Namespace
        Training configuration options parsed from config file and command line.
    kcordict : dict
        K-correction dictionary containing filter information and photometric
        systems for each survey.
    surveylist : list
        List of surveys included in the training.

    See Also
    --------
    TrainSALTBase : Base class providing configuration and data handling.
    RunTraining : Entry point that instantiates and runs TrainSALT.
    """

    def initialParameters(self, datadict):
        """
        Initialize model parameters and construct parameter list.

        Sets up initial guesses for all SALT3 model parameters including
        M0/M1 spectral components, color law coefficients, error surfaces,
        and per-SN parameters (x0, x1, c, spectral recalibration).

        Parameters
        ----------
        datadict : dict
            Dictionary of training data, keyed by SN ID. Each entry is a
            SALTtrainingdata object containing photometry and spectroscopy.

        Returns
        -------
        guess : ndarray
            Initial parameter values for the optimizer.
        modelconfiguration : saltconfiguration
            Configuration object containing parameter list and knot locations.
        """
        from saltshaker.initfiles import init_rootdir
        ...
        if self.options.initm0modelfile and not os.path.exists(
            self.options.initm0modelfile
        ):
            if self.options.initm0modelfile:
                self.options.initm0modelfile = (
                    f"{init_rootdir}/{self.options.initm0modelfile}"
                )
            if self.options.initm1modelfile:
                self.options.initm1modelfile = (
                    f"{init_rootdir}/{self.options.initm1modelfile}"
                )
        ...
        if self.options.initm0modelfile and not os.path.exists(
            ...
            raise RuntimeError(
                "model initialization file not found in local directory or %s"
                % init_rootdir
            )
        ...
        init_options = {
            ...
            "phasesplineres": self.options.phasesplineres,
            "wavesplineres": self.options.wavesplineres,
            "phaseinterpres": self.options.phaseinterpres,
            "waveinterpres": self.options.waveinterpres,
            "normalize": True,
            "order": self.options.bsorder,
            "use_snpca_knots": self.options.use_snpca_knots,
        }
        ...
        if self.options.initm0modelfile and self.options.initm1modelfile:
            if self.options.initsalt2model:
                phase, wave, m0, m1, phaseknotloc, waveknotloc, m0knots, m1knots = (
                    init_salt2(
                        m0file=self.options.initm0modelfile,
                        m1file=self.options.initm1modelfile,
                        Bfilt=self.options.initbfilt,
                        flatnu=_flatnu,
                        **init_options,
                    )
                )
        ...
        if self.options.initsalt2var:
            ...
            if not self.options.host_component:
                (
                    ...
                    m0m1corrknots,
                    clscatcoeffs,
                ) = init_errs_percent(**init_options)
            else:
                (
                    errphaseknotloc,
                    errwaveknotloc,
        ...
        n_phaseknots, n_waveknots = (
            len(phaseknotloc) - self.options.bsorder - 1,
            len(waveknotloc) - self.options.bsorder - 1,
        )
        n_errphaseknots, n_errwaveknots = (
            len(errphaseknotloc) - self.options.errbsorder - 1,
            len(errwaveknotloc) - self.options.errbsorder - 1,
        )
        ...
        if self.options.n_colorpars:
            parlist = np.append(
                parlist,
                [[f"cl{i}"] * num for i, num in enumerate(self.options.n_colorpars)],
            )
        if (
            self.options.error_snake_phase_binsize
            and self.options.error_snake_wave_binsize
        ):
            for i in range(self.options.n_errorsurfaces):
                parlist = np.append(
                    parlist,
                    ["modelerr_{}".format(i)] * n_errphaseknots * n_errwaveknots,
                )
                for j in range(i):
                    parlist = np.append(
                        parlist,
                        [f"modelcorr_{j}{i}"] * n_errphaseknots * n_errwaveknots,
                    )
            for i in range(self.options.n_errorsurfaces):
                parlist = np.append(
                    parlist,
                    ["specmodelerr_{}".format(i)] * n_errphaseknots * n_errwaveknots,
                )
                for j in range(i):
                    parlist = np.append(
                        parlist,
                        [f"specmodelcorr_{j}{i}"] * n_errphaseknots * n_errwaveknots,
                    )
            if self.options.host_component:
        ...
        parlist = np.append(
            ...
            sum(
                [
                    [f"x{i}_{k}" for i in range(self.options.n_components)]
                    + [f"c{i}_{k}" for i in range(len(self.options.n_colorpars))]
        ...
        for sn in datadict.keys():
            ...
            for k in specdata.keys():
                order = self.options.n_min_specrecal + int(
                    np.log(
                        (specdata[k].wavelength.max() - specdata[k].wavelength.min())
                        / self.options.specrange_wavescale_specrecal
                    )
                    + len(photdata) * self.options.n_specrecal_per_lightcurve
                )
                order = min(
                    max(order, self.options.n_min_specrecal),
                    self.options.n_max_specrecal,
                )

                # save the order as part of the specrecal list
                if (
                    not self.options.specrecallist
                    or sn not in spcrcldata["SNID"]
                    or k + 1 not in spcrcldata["N"][spcrcldata["SNID"] == sn]
                ):
                    datadict[sn].specdata[k].n_specrecal = order
                # if datadict[sn].specdata[k].n_specrecal is None:
                #    import pdb; pdb.set_trace()
                recalParams = [f"specx0_{sn}_{k}"] + [f"specrecal_{sn}_{k}"] * (
                    order - 1
                )
        ...
        if self.options.usesurverrfloors:
            filtlist = set(
                sum([list(sn.photdata.keys()) for sn in datadict.values()], [])
            )
            parlist = np.append(parlist, [f"surverrfloor_{x}" for x in filtlist])

        modelconfiguration = saltresids.saltconfiguration(
            parlist=parlist,
            phaseknotloc=phaseknotloc,
            waveknotloc=waveknotloc,
            errphaseknotloc=errphaseknotloc,
            errwaveknotloc=errwaveknotloc,
        )
        # initial guesses
        ...
        if self.options.resume_from_outputdir:
            log.info(
                f"resuming from output directory {self.options.resume_from_outputdir}"
            )

            names = None
            for possibleDir in [
                self.options.resume_from_outputdir,
                self.options.outputdir,
            ]:
                for possibleFile in [
                    "salt3_parameters_unscaled.dat",
                    "salt3_parameters.dat",
                ]:
                    if names is None:
                        try:
                            names, pars = np.loadtxt(
                                path.join(possibleDir, possibleFile),
                                unpack=True,
                                skiprows=1,
                                dtype="U30,f8",
                            )
            ...
            if self.options.resume_from_gnhistory:
                with open(
                    f"{self.options.resume_from_gnhistory}/gaussnewtonhistory.pickle",
                    "rb",
                ) as fin:
            ...
            for key in np.unique(parlist):
                try:
                    ...
                    print(key)
                    log.critical(
                        f"Problem while initializing parameter {key} from previous training"
                    )
                    sys.exit(1)
        else:
            ...
            for i in range(3):
                guess[parlist == "modelerr_{}".format(i)] = 1e-6
            ...
            if self.options.n_colorpars:
                if self.options.initsalt2model:
                    ...
                    if self.options.n_colorpars == [4]:
                        ...
                        clwave = np.linspace(
                            self.options.waverange[0], self.options.waverange[1], 1000
                        )
                        salt2cl = SALT2ColorLaw(
                            [2800.0, 7000.0],
                            [-0.504294, 0.787691, -0.461715, 0.0815619],
                        )(clwave)
                        initcolorlaw = colorlaw.getcolorlaw(
                            self.options.colorlaw_function[0]
                        )(self.options.n_colorpars[0], self.options.colorwaverange)
                        ...
                        md = least_squares(
                            bestfit, np.zeros(self.options.n_colorpars[0])
                        )
                        if (
                            "termination conditions are satisfied" not in md.message
                            and "termination condition is satisfied" not in md.message
                        ):

                            raise RuntimeError("problem initializing color law!")
                        guess[parlist == "cl0"] = md.x
                else:
                    guess[parlist == "cl0"] = [0.0] * self.options.n_colorpars
            if self.options.n_colorscatpars:
            ...
            if self.options.host_component:
                guess[parlist == "modelerr_host"] = (
                    1e-9  # something small...  #mhostvarknots
                )
            ...
            for sn in datadict.keys():
                if self.options.snparlist:
                    ...
                    iSN = (
                        (sn == snpar["SNID"])
                        | ("sn" + sn == snpar["SNID"])
                        | ("sn" + sn.lower() == snpar["SNID"])
                        | (sn + ".0" == snpar["SNID"])
                    )
                    if len(snpar["SNID"][iSN]) > 1:
                        raise RuntimeError(
                            f"found duplicate in parameter list for SN {snpar['SNID'][iSN][0]}"
                        )
                    if len(snpar[iSN]):
                        if self.options.host_component:
                        ...
                        for i in range((self.options.n_components)):
                            if ("x" + str(i)) in snpar.keys():
                                guess[parlist == f"x{i}_{sn}"] = snpar["x" + str(i)][
                                    iSN
                                ]
                            else:
                        ...
                        if snpar["x0"][iSN] <= 0:
                            log.warning(
                                f'Bad input value for {sn}: x0={ float(snpar["x0"][iSN])}'
                            )
                            guess[parlist == f"x0_{sn}"] = 10 ** (
                                -0.4
                                * (
                                    cosmo.distmod(datadict[sn].zHelio).value
                                    - 19.36
                                    - 10.635
                                )
                            )
                        ...
                        log.warning(
                            f"SN {sn} not found in SN par list {self.options.snparlist}"
                        )
                        guess[parlist == "x0_%s" % sn] = 10 ** (
                            -0.4
                            * (
                                cosmo.distmod(datadict[sn].zHelio).value
                                - 19.36
                    ...
                    log.info(
                        f"initializing parameters using simulated values for SN {sn}"
                    )
                    ...
                    guess[parlist == "x0_%s" % sn] = 10 ** (
                        -0.4
                        * (cosmo.distmod(datadict[sn].zHelio).value - 19.36 - 10.635)
                    )

                for k in datadict[sn].specdata:
                    guess[parlist == f"specx0_{sn}_{k}"] = guess[
                        parlist == "x0_%s" % sn
                    ]
            # let's redefine x1 before we start
            ratio = _RatioToSatisfyDefinitions(phase, wave, self.kcordict, [m0, m1])
            ix1 = np.array(
                [i for i, si in enumerate(parlist) if si.startswith("x1")], dtype=int
            )
            ...
            for sn in datadict.keys():
                ...
                for k in specdata.keys():
                    ...
                    recalCoord = (
                        specdata[k].wavelength - np.mean(specdata[k].wavelength)
                    ) / 2500
                    drecaltermdrecal = (
                    ...
                    zHel, x0, x1, c = (
                        datadict[sn].zHelio,
                        guess[parlist == f"x0_{sn}"],
                        guess[parlist == f"x1_{sn}"],
                        guess[parlist == f"c_{sn}"],
                    )
                    mwebv = datadict[sn].MWEBV

                    uncalledModel = _specflux(
                        specdata[k].tobs,
                        specdata[k].wavelength,
                        phase,
                        wave,
                        m0,
                        m1,
                        lambda wave: initcolorlaw(1, guess[parlist == "cl0"], wave),
                        zHel,
                        x0,
                        x1,
                        c,
                    ...
                    def recalpars(x):
                        recalexp = np.exp(
                            (drecaltermdrecal * x[1:][np.newaxis, :]).sum(axis=1)
                        )
                        return (
                            x[0] * uncalledModel * recalexp - specdata[k].flux
                        ) / specdata[k].fluxerr

                    md = least_squares(
                        recalpars,
                        [np.median(specdata[k].flux) / np.median(uncalledModel)]
                        + list(guess[parlist == "specrecal_{}_{}".format(sn, k)]),
                    )
        ...
        if self.options.fix_salt2components_initdir:
            log.info(
                f"resuming from output directory {self.options.fix_salt2components_initdir}"
            )
            ...
            for key in np.unique(parlist):
                try:
                    ...
                    log.info(
                        f"Could not initializing parameter {key} from previous training"
                    )
                    pass
    ...
    def bootstrapSALTModel_batch(self, datadict, trainingresult, saltfitter):
        ...
        if not self.options.get_bootstrap_output_only:
            for i in range(self.options.n_bootstrap):
                ...
                with open(sbatch_file) as fin, open(
                    f"saltshaker_batch_{i}", "w"
                ) as fout:
                    for line in fin:
                        line = line.replace("\n", "")
                        print(
                            line.replace("REPLACE_JOB", cmd)
                            ...
                            .replace("REPLACE_CPUS_PER_TASK", "1"),
                            file=fout,
                        )
        ...
        while unfinished_jobs:
            ...
            for p in pid_all:
                if p not in pid_init and p in pid_new:
                    unfinished_jobs = True
        ...
        M0_bs, M1_bs, Mhost_bs = (
            np.zeros(
                [
                    np.shape(trainingresult.M0)[0],
                    np.shape(trainingresult.M0)[1],
        ...
        for i in range(self.options.n_bootstrap):
            if not os.path.exists(
                f"{self.options.outputdir}/bootstrap_{i}/salt3_parameters.dat"
            ):

                continue
            iGood = np.append(iGood, i)
            try:
                params, parvals = np.loadtxt(
                    f"{self.options.outputdir}/bootstrap_{i}/salt3_parameters.dat",
                    unpack=True,
                    dtype=str,
                    skiprows=1,
            ...
            # save each result
            if not self.options.host_component:
                M0, M1 = saltfitter.SALTModel(
                    parvals,
                    evaluatePhase=saltfitter.phaseout,
                    evaluateWave=saltfitter.waveout,
                )
            else:
                M0, M1, Mhost = saltfitter.SALTModel(
                    parvals,
                    evaluatePhase=saltfitter.phaseout,
                    evaluateWave=saltfitter.waveout,
                )
            M0_bs[:, :, i] = (
                M0  # parvals[params == 'm0'].reshape(np.shape(trainingresult.M0))
            )
            M1_bs[:, :, i] = (
                M1  # parvals[params == 'm1'].reshape(np.shape(trainingresult.M0))
            )
            if self.options.host_component and len(parvals[params == "mhost"]):
                Mhost_bs[:, :, i] = (
                    Mhost  # parvals[params == 'mhost'].reshape(np.shape(trainingresult.M0))
                )
        ...
        for j in range(np.shape(M0_bs)[0]):
            for i in range(np.shape(M0_bs)[1]):
                trainingresult.cov_M0_M1_bootstrap[j, i] = np.sum(
                    (M0_bs[j, i] - np.mean(M0_bs[j, i, iGood]))
                    * (M1_bs[j, i] - np.mean(M1_bs[j, i, iGood]))
                ) / (self.options.n_bootstrap - 1)
                trainingresult.cov_M0_Mhost_bootstrap[j, i] = np.sum(
                    (M0_bs[j, i] - np.mean(M0_bs[j, i, iGood]))
                    * (Mhost_bs[j, i] - np.mean(Mhost_bs[j, i, iGood]))
                ) / (self.options.n_bootstrap - 1)
    ...
    def bootstrapSALTModel(self, datadict, trainingresult):
        ...
        if (
            self.options.use_previous_errors
            and not self.options.resume_from_outputdir
            and not self.options.error_dir
        ):
            raise RuntimeError(
                "resume_from_outputdir or error_dir must be specified to use use_previous_errors option"
            )

        x_modelpars, phaseknotloc, waveknotloc, errphaseknotloc, errwaveknotloc = (
            self.initialParameters(datadict)
        )

        saltfitkwargs = self.get_saltkw(
            phaseknotloc, waveknotloc, errphaseknotloc, errwaveknotloc
        )
        n_phaseknots, n_waveknots = len(phaseknotloc) - 4, len(waveknotloc) - 4
        n_errphaseknots, n_errwaveknots = (
            len(errphaseknotloc) - 4,
            len(errwaveknotloc) - 4,
        )
        ...
        for j in range(self.options.n_bootstrap):
            new_keys = np.random.choice(
                list(datadict.keys()), size=len(datadict.keys())
            )
            ...
            x_modelpars_bs, parlist_bs, keys_done = (
                np.array([]),
                np.array([]),
                np.array([]),
            )
            for i, xm in enumerate(x_modelpars):
                if (
                    "_" not in parlist[i]
                    or "modelerr" in parlist[i]
                    or "modelcorr" in parlist[i]
                ):
            ...
            for i, k in enumerate(new_keys):
                snidpars = [
                    (x, p)
                    for x, p in zip(x_modelpars, parlist)
                    if "_" in p and p.split("_")[1] == k
                ]
                for xp in snidpars:
                    x, p = xp
                    ...
                    if len(parlist_parts) == 2:
                        x_modelpars_bs = np.append(
                            x_modelpars_bs, x
                        )  # x_modelpars[parlist == p])
                        parlist_bs = np.append(
                            parlist_bs, parlist_parts[0] + "_" + str(i)
                        )
                        ...
                        parlist_bs = np.append(
                            parlist_bs,
                            parlist_parts[0] + "_" + str(i) + "_" + parlist_parts[2],
                        )
            ...
            saltfitter = saltfit.GaussNewton(
                x_modelpars_bs, datadict_bootstrap, parlist_bs, **saltfitkwargs
            )
            ...
            trainingresult_bs, message = fitter.gaussnewton(
                saltfitter,
                x_modelpars_bs,
                self.options.maxiter_bootstrap,
            ...
            saltfitter.maxlikefit(trainingresult_bs.params_raw)
        ...
        for j in range(np.shape(M0_bs)[0]):
            for i in range(np.shape(M0_bs)[1]):
                trainingresult.cov_M0_M1_bootstrap[j, i] = np.sum(
                    (M0_bs[j, i] - np.mean(M0_bs[j, i, :]))
                    * (M1_bs[j, i] - np.mean(M1_bs[j, i, :]))
                ) / (self.options.n_bootstrap - 1)
                trainingresult.cov_M0_Mhost_bootstrap[j, i] = np.sum(
                    (M0_bs[j, i] - np.mean(M0_bs[j, i, :]))
                    * (Mhost_bs[j, i] - np.mean(Mhost_bs[j, i, :]))
                ) / (self.options.n_bootstrap - 1)
    ...
    def initializesaltmodelobject(self, datadict):
        ...
        return x_modelpars, saltresids.SALTResids(
            datadict, self.kcordict, modelconfiguration, self.options
        )

    def fitSALTModel(self, datadict, x_modelpars, saltresids, return_optimizer=True):
        """
        Fit the SALT3 model to training data.

        Runs the optimization algorithm (typically Gauss-Newton) to fit
        model parameters to the photometric and spectroscopic training data.

        Parameters
        ----------
        datadict : dict
            Dictionary of training data keyed by SN ID.
        x_modelpars : ndarray
            Initial parameter values.
        saltresids : SALTResids
            Residual calculator object for computing chi-squared.
        return_optimizer : bool, optional
            If True, return the optimizer object for further use.
            Default is True.

        Returns
        -------
        trainingresult : TrainingResult
            Object containing fitted parameters, model surfaces, and metadata.
        chain : ndarray or None
            MCMC chain if applicable, else None.
        loglikes : ndarray or None
            Log-likelihood values if applicable, else None.
        saltfitter : optimizer
            The optimizer object used for fitting.
        """
        # check for option inconsistency
        if (
            self.options.use_previous_errors
        ...
        if self.options.bootstrap_single:
            new_keys = np.random.choice(
                list(datadict.keys()), size=len(datadict.keys()), replace=True
            )
        ...
        for i in range(self.options.n_repeat):
            ...
            saltfitter = optimizer(
                x_modelpars, saltresids, self.options.outputdir, self.options
            )
            if return_optimizer:
        ...
        if "chain" in saltfitter.__dict__.keys():
            ...
            chain, loglikes = None, None

        return trainingresult, chain, loglikes, saltfitter

    def wrtoutput(self, outdir, trainingresult, chain, loglikes, datadict):
        """
        Write trained model outputs to disk.

        Generates all SALT3 model files including spectral templates,
        color law, error surfaces, covariance matrices, and SN parameters.
        ...
        trainingresult : TrainingResult
            Fitted model parameters and surfaces.
        chain : ndarray or None
            MCMC chain if available.
        loglikes : ndarray or None
            Log-likelihood values if available.
        datadict : dict
            Training data dictionary for SN parameter output.

        Notes
        -----
        Output files include:
        - salt3_template_0.dat, salt3_template_1.dat : M0, M1 spectral surfaces
        - salt3_color_correction.dat : Color law coefficients
        - salt3_lc_variance_*.dat : Model variance surfaces
        - salt3_lc_covariance_*.dat : Model covariance surfaces
        - salt3_color_dispersion.dat : Color scatter model
        - salt3train_snparams.txt : Best-fit SN parameters
        - SALT3.INFO : Model metadata
        """
        ...
        if trainingresult.dataerrsurfaces is None:
            dataerrsurfaces = [
                np.zeros((trainingresult.phase.size, trainingresult.wave.size))
                for j in range(len(trainingresult.componentnames))
            ]
        else:
        ...
        for fluxmodel, errmodel, errdata, name in zip(
            trainingresult.componentsurfaces,
            trainingresult.modelerrsurfaces,
            dataerrsurfaces,
            trainingresult.componentnames,
        ):
            with open(
                f"{outdir}/salt3_template_{name[1:]}.dat", "w"
            ) as foutmodel, open(
                f"{outdir}/salt3_lc_model_variance_{name[1:]}.dat", "w"
            ) as foutmodelerr, open(
                f"{outdir}/salt3_lc_variance_{name[1:]}.dat", "w"
            ) as foutdataerr:
                for i, p in enumerate(trainingresult.phase):
                    for j, w in enumerate(trainingresult.wave):
                        ...
                        if not self.options.use_previous_errors:
                            foutmodelerr.write(
                                f"{p:.1f} {w:.2f} {errmodel[i,j]**2.:8.15e}\n"
                            )
                            foutdataerr.write(
                                f"{p:.1f} {w:.2f} {errmodel[i,j]**2.+errdata[i,j]**2.:8.15e}\n"
                            )
        ...
        if self.options.use_previous_errors:
            if self.options.resume_from_outputdir:
                ...
                log.critical(
                    '"use_previous_errors" enabled, but no directory given to pull errors from'
                )
            for filename in os.listdir(errordir):
                if "variance" in filename and "salt3" in filename:
                    try:
                        os.system(f"cp {errordir}/{filename} {outdir}/{filename}")
                    except:
                        pass
        else:
            ...
            for firstind, secondind, covsurface in trainingresult.modelcovsurfaces:
                ...
                with open(
                    f"{outdir}/salt3_lc_model_covariance_{trainingresult.componentnames[firstind][1:]}{trainingresult.componentnames[secondind][1:]}.dat",
                    "w",
                ) as foutcov:
                    for i, p in enumerate(trainingresult.phase):
                        for j, w in enumerate(trainingresult.wave):
            ...
            for firstind, secondind, datasurface in datacovsurfaces:
                ...
                with open(
                    f"{outdir}/salt3_lc_covariance_{trainingresult.componentnames[firstind][1:]}{trainingresult.componentnames[secondind][1:]}.dat",
                    "w",
                ) as foutcov:
                    for i, p in enumerate(trainingresult.phase):
                        for j, w in enumerate(trainingresult.wave):
                            foutcov.write(
                                f"{p:.1f} {w:.2f} {modelsurface[i,j]+datasurface[i,j]:8.15e}\n"
                            )
        ...
        with open(f"{outdir}/salt3_color_dispersion.dat", "w") as foutclscat:
            for j, w in enumerate(trainingresult.wave):
                print(
                    f"{w:.2f} {np.clip(trainingresult.clscat[j],0.,cldispersionmax):8.15e}",
                    file=foutclscat,
                )
...
SIGMA_INT: 0.106  # used in simulation"""
        ...
        if len(trainingresult.clpars) == 1:
            ...
            with open(f"{outdir}/salt3_color_correction.dat", "w") as foutcl:
                ...
                print(
                    f"""Salt2ExtinctionLaw.version 1
Salt2ExtinctionLaw.min_lambda {self.options.colorwaverange[0]:.0f}
Salt2ExtinctionLaw.max_lambda {self.options.colorwaverange[1]:.0f}""",
                    file=foutcl,
                )
        else:
            for i, name, colorlaw in zip(
                range(trainingresult.clpars.shape[0]),
                self.options.colorlaw_function,
                trainingresult.clpars,
            ):
...
Salt2ExtinctionLaw.max_lambda {self.options.colorwaverange[1]:.0f}""",
        ...
        with open(f"{outdir}/salt3train_snparams.txt", "w") as foutsn:
            print(
                "# SN x0 x1 c t0 SIM_x0 SIM_x1 SIM_c SIM_t0 SALT2_x0 SALT2_x1 SALT2_c SALT2_t0",
                file=foutsn,
            )
            ...
            for snlist in self.options.snlists.split(","):
                ...
                if not os.path.exists(snlist):
                    log.warning(
                        f"SN list file {snlist} does not exist. Checking {data_rootdir}/trainingdata/{snlist}"
                    )
                ...
                for k in trainingresult.snparams.keys():
                    ...
                    for l in snfiles:
                        ...
                        if "SIM_SALT2c" in sn.__dict__.keys():
                            ...
                            SIM_c = -99
                        if "SIM_PEAKMJD" in sn.__dict__.keys():
                            SIM_PEAKMJD = float(sn.SIM_PEAKMJD.split()[0])
                        else:
                            SIM_PEAKMJD = -99
                        break
                    if not foundfile:
                    ...
                    print(
                        f"{k} {trainingresult.snparams[k]['x0']:8.10e} {trainingresult.snparams[k]['x1']:.10f} {trainingresult.snparams[k]['c'] if 'c' in trainingresult.snparams[k] else trainingresult.snparams[k]['c0']:.10f} {trainingresult.snparams[k]['t0']:.10f} {SIM_x0:8.10e} {SIM_x1:.10f} {SIM_c:.10f} {SIM_PEAKMJD:.2f} {salt2x0:8.10e} {salt2x1:.10f} {salt2c:.10f} {salt2t0:.10f}",
                        file=foutsn,
                    )
        ...
        with open(f"{self.options.yamloutputfile}", "w") as file:
            yaml.dump(yamloutputdict, file)
    ...
    def salt2fit(self, sn, datadict):
        ...
        for flt in np.unique(sn.FLT):
            ...
            band = sncosmo.Bandpass(filtwave, filttrans, wave_unit=u.angstrom, name=flt)
        ...
        for m, flt, flx, flxe in zip(sn.MJD, sn.FLT, sn.FLUXCAL, sn.FLUXCALERR):
            if self.kcordict[sn.SURVEY][flt]["magsys"] == "BD17":
                sys = "bd17"
            elif self.kcordict[sn.SURVEY][flt]["magsys"] == "AB":
                sys = "ab"
            else:
                sys = "vega"
            if (
                self.kcordict[sn.SURVEY][flt]["lambdaeff"]
                / (1 + float(sn.REDSHIFT_HELIO.split("+-")[0]))
                > 2000
                and self.kcordict[sn.SURVEY][flt]["lambdaeff"]
                / (1 + float(sn.REDSHIFT_HELIO.split("+-")[0]))
                < 9200
        ...
        result, fitted_model = sncosmo.fit_lc(
            ...
            bounds={
                "t0": (
                    sn.MJD[sn.FLUXCAL == np.max(sn.FLUXCAL)][0] - 10,
                    sn.MJD[sn.FLUXCAL == np.max(sn.FLUXCAL)][0] + 10,
                ),
                "z": (0.0, 0.7),
                "x1": (-3, 3),
    ...
    def validate(self, outputdir, datadict, modelonly=False):
        """
        Run validation diagnostics on trained model.

        Generates diagnostic plots comparing the trained model to data
        and to SALT2, including light curve fits, spectral comparisons,
        parameter distributions, and model surface visualizations.

        Parameters
        ----------
        outputdir : str
            Directory containing trained model output files.
        datadict : dict
            Training data dictionary.
        modelonly : bool, optional
            If True, only generate model plots without light curve fits.
            Default is False.

        Notes
        -----
        Generated outputs include:
        - SALTmodelcomp.png : M0, M1 surface comparison with SALT2
        - SALTmodelerrcomp.pdf : Error surface comparison
        - lcfits.pdf : Light curve fit residuals for all SNe
        - speccomp_*.pdf : Spectral comparison plots
        - saltparcomp.png : Parameter comparison (if fitsalt2=True)
        """
        # prelims
        plt.subplots_adjust(
            left=None, bottom=None, right=None, top=0.85, wspace=0.025, hspace=0
        )
        x0, x1, c, t0 = np.loadtxt(
            f"{outputdir}/salt3train_snparams.txt", unpack=True, usecols=[1, 2, 3, 4]
        )
        snid = np.genfromtxt(
            f"{outputdir}/salt3train_snparams.txt",
            unpack=True,
            dtype="str",
            usecols=[0],
        )
        ...
        if self.options.fitsalt2:
            ValidateParams.main(
                f"{outputdir}/salt3train_snparams.txt", f"{outputdir}/saltparcomp.png"
            )

        plotSALTModel.mkModelErrPlot(
            outputdir,
            outfile=f"{outputdir}/SALTmodelerrcomp.pdf",
            xlimits=[self.options.waverange[0], self.options.waverange[1]],
        )

        plotSALTModel.mkModelPlot(
            outputdir,
            outfile=f"{outputdir}/SALTmodelcomp.png",
            xlimits=[self.options.waverange[0], self.options.waverange[1]],
        ...
        for j, snlist in enumerate(self.options.snlists.split(",")):
            ...
            parlist, parameters = np.genfromtxt(
                f"{outputdir}/salt3_parameters.dat",
                unpack=True,
                dtype=str,
                skip_header=1,
            )
            parameters = parameters.astype(float)
            CheckSALTParams.checkSALT(
                parameters, parlist, snfiles, snlist, outputdir, idx=j
            )
        ...
        for k in self.kcordict.keys():
            ...
            for k2 in self.kcordict[k].keys():
                if k2 not in [
                    ...
                    if self.kcordict[k][k2]["magsys"] == "AB":
                        primarykey = "AB"
                    elif self.kcordict[k][k2]["magsys"] == "Vega":
                        primarykey = "Vega"
                    elif self.kcordict[k][k2]["magsys"] == "VEGA":
                        primarykey = "Vega"
                    elif self.kcordict[k][k2]["magsys"] == "BD17":
                        primarykey = "BD17"
        ...
        salt3phase, salt3wave, salt3flux = np.genfromtxt(
            f"{outputdir}/{m0file}", unpack=True
        )
        salt3m1phase, salt3m1wave, salt3m1flux = np.genfromtxt(
            f"{outputdir}/{m1file}", unpack=True
        )
        salt2phase, salt2wave, salt2flux = np.genfromtxt(
            f"{salt2dir}/salt2_template_0.dat", unpack=True
        )
        salt2m1phase, salt2m1wave, salt2m1flux = np.genfromtxt(
            f"{salt2dir}/salt2_template_1.dat", unpack=True
        )
        ...
        saltdict = {
            "salt3phase": salt3phase,
            "salt3wave": salt3wave,
            "salt3flux": salt3flux,
            "salt3m1phase": salt3m1phase,
            "salt3m1wave": salt3m1wave,
            "salt3m1flux": salt3m1flux,
            "salt2phase": salt2phase,
            "salt2wave": salt2wave,
            "salt2m0flux": salt2m0flux,
            "salt2m1phase": salt2m1phase,
            "salt2m1wave": salt2m1wave,
            "salt2m1flux": salt2m1flux,
        }
        ...
        for j, snlist in enumerate(self.options.snlists.split(",")):
            ...
            if self.options.dospec:
                ...
                ValidateSpectra.compareSpectra(
                    snlist,
                    self.options.outputdir,
                    specfile=f"{self.options.outputdir}/speccomp_{j:.0f}.pdf",
                    maxspec=2000,
                    base=self,
            ...
            for l in snfiles:
                if l.lower().endswith(".fits") or l.lower().endswith(".fits.gz"):
                    ...
                    if (
                        l.lower().endswith(".fits")
                        and not os.path.exists(l)
                        and os.path.exists("{}.gz".format(l))
                    ):
                        l = "{}.gz".format(l)
                    # get list of SNIDs
                    ...
                    for sniditer in snidlist:
                        sn = snana.SuperNova(
                            snid=sniditer,
                            headfitsfile=l,
                            photfitsfile=l.replace("_HEAD.FITS", "_PHOT.FITS"),
                            specfitsfile=None,
                            readspec=False,
                        )
                        sn.SNID = str(sn.SNID)
            ...
            for sn, l in zip(plotsnlist, snfilelist):
                ...
                if not i % 15:
                    fig = plt.figure()
                try:
                    ax1 = plt.subplot(gs1[i % 15])
                    ax2 = plt.subplot(gs1[(i + 1) % 15])
                    ax3 = plt.subplot(gs1[(i + 2) % 15])
                    ax4 = plt.subplot(gs1[(i + 3) % 15])
                    ax5 = plt.subplot(gs1[(i + 4) % 15])
                except:
                    import pdb
                ...
                salt2chi2, salt3chi2 = ValidateLightcurves.customfilt(
                    f"{outputdir}/lccomp_{sn.SNID}.png",
                    l,
                    outputdir,
                    t0=t0sn,
                    x0=x0sn,
            ...
            log.info(
                f"total chi^2 is {salt2_chi2tot:.1f} for SALT2 and {salt3_chi2tot:.1f} for SALT3"
            )
        if not i % 15 == 0:
            pdf_pages.savefig()
        pdf_pages.close()
    ...
    def main(self, return_optimizer=False):
        """
        Execute the full SALT3 training workflow.

        Orchestrates the complete training process: loading data, applying
        cuts, initializing parameters, fitting the model, writing outputs,
        and running validation.

        Parameters
        ----------
        return_optimizer : bool, optional
            If True, return the optimizer object instead of running the
            full training. Useful for debugging. Default is False.

        Returns
        -------
        None or tuple
            If return_optimizer is True, returns (fitter, saltfitter, modelpars).
            Otherwise returns None.

        Raises
        ------
        RuntimeError
            If training fails during initialization, training, or output stages.
        """
        try:
            ...
            for snid, sn in datadict.items():
                for filt in sn.filt:
                    if filt not in self.kcordict[sn.survey]:
                        if filt not in self.options.__dict__[
                            f"{sn.survey.split('(')[0]}_ignore_filters"
                        ].replace(" ", "").split(","):
                            raise ValueError(
                                f'Kcor file missing key {filt} from survey {sn.survey} for sn {snid}; valid keys are {", ".join([x for x in self.kcordict[sn.survey] if "lambdaeff" in self.kcordict[sn.survey][x]])}'
                            )
            datadict = self.mkcuts(datadict)[0]
            log.info(f"took {time.time()-tcstart:.3f} to apply cuts")

            phasebins = np.linspace(
                *self.options.phaserange,
                int(
                    (self.options.phaserange[1] - self.options.phaserange[0])
                    / self.options.phasesplineres
                )
                + 1,
                True,
            )
            wavebins = np.linspace(
                *self.options.waverange,
                int(
                    (self.options.waverange[1] - self.options.waverange[0])
                    / self.options.wavesplineres
                )
                + 1,
                True,
            )
            datadensity.datadensityplot(
                path.join(self.options.outputdir, "datadensity.pdf"),
                phasebins,
                wavebins,
                datadict,
                self.kcordict,
            ...
            if self.options.stage == "all" or self.options.stage == "train":
                ...
                if not return_optimizer:
                    trainingresult, chain, loglikes, saltfitter = self.fitSALTModel(
                        datadict,
                        x_modelpars,
                        saltresids,
                    ...
                    fitter, saltfitter, modelpars = self.fitSALTModel(
                        datadict,
                        x_modelpars,
                        saltresids,
                        return_optimizer=return_optimizer,
                ...
                if self.options.errors_from_bootstrap:
                    if self.options.bootstrap_batch_mode:
                        trainingresult = self.bootstrapSALTModel_batch(
                            datadict, trainingresult, x_modelpars, saltresids
                        )
                    else:
                        trainingresult = self.bootstrapSALTModel(
                            datadict, trainingresult, x_modelpars, saltresids
                        )
                ...
                self.wrtoutput(
                    self.options.outputdir, trainingresult, chain, loglikes, datadict
                )
            log.info(
                "successful SALT3 training!  Output files written to %s"
                % self.options.outputdir
            )
    ...
    def createGaussNewton(self):
        ...
        fitter, saltfitter, modelpars = self.main(return_optimizer=True)
...
class RunTraining:
    """
    Entry point for SALT3 model training.

    Handles command-line argument parsing, configuration file loading,
    and orchestration of the training process. This is the class invoked
    by the `trainsalt` command-line script.
...
config file options can be overwridden at the command line"""
    ...
    def get_config_options(self, salt, configfile, args=None):
        ...
        loggerconfig.dictconfigfromYAML(
            user_options.loggingconfig, user_options.outputdir
        )
        if not os.path.exists(user_options.modelconfig):
            print(
                "warning : model config file %s doesn't exist.  Trying package directory"
                % user_options.modelconfig
            )
            user_options.modelconfig = "%s/%s" % (
                config_rootdir,
                user_options.modelconfig,
            )
        if not os.path.exists(user_options.modelconfig):
            raise RuntimeError(
                "can't find model config file!  Checked %s" % user_options.modelconfig
            )

        if not os.path.exists(user_options.trainingconfig):
            print(
                "warning : trainingconfig config file %s doesn't exist.  Trying package directory"
                % user_options.trainingconfig
            )
            user_options.trainingconfig = "%s/%s" % (
                config_rootdir,
                user_options.trainingconfig,
            )
        if not os.path.exists(user_options.trainingconfig):
            raise RuntimeError(
                "can't find trainingconfig config file!  Checked %s"
                % user_options.trainingconfig
            )
    ...
    def get_example_data(self):
        ...
        if os.path.exists("saltshaker-latest-training"):
            raise RuntimeError(
                "saltshaker-latest-training exists in local directory.  Please remove it before continuing"
            )
        download_dir(_example_data_url, os.getcwd())

    def main(self, configfile=None, args=None):
        """
        Run SALT3 training with given configuration.

        Parameters
        ----------
        configfile : str, optional
            Path to configuration file. If None, parsed from command line.
        ...
        if configfile is None:
            ...
            parser = argparse.ArgumentParser(
                usage=self.usagestring, conflict_handler="resolve", add_help=False
            )
            parser.add_argument(
                "configpositional",
                nargs="?",
            ...
            parser.add_argument(
                "-c", "--configfile", default=None, type=str, help="configuration file"
            )
            parser.add_argument(
                "-g",
                "--get-example-data",
        ...
        if options.get_example_data:
            ...
            print(
                "example data has been downloaded to the saltshaker-latest-training directory"
            )
        else:
            if configfile is None and configpos is not None:
                ...
                raise RuntimeError(
                    "Configuration file must be specified at command line"
                )

#!/usr/bin/env python
"""
Spectral synthetic photometry comparison utilities.

This module provides functions to compare spectra with photometry by computing
synthetic photometry and deriving scale factors or color differences. Used
during data preparation to assess spectrophotometric consistency.

Functions
---------
getScaleForSN
    Compute scale factor to match spectrum to photometry.
getColorsForSN
    Compute color differences between spectrum and photometry.
"""
# D. Jones - 11/1/2019
...
def getScaleForSN(spectrum,photdata,kcordict,survey,colorcut=0.1):
    """
    Compute scale factor to match spectrum synthetic photometry to observed photometry.

    Parameters
    ----------
    spectrum : dict
        Spectrum data with keys 'wavelength', 'flux', 'fluxerr', 'mjd'.
    photdata : dict
        Photometric data with keys 'mjd', 'filt', 'fluxcal', 'fluxcalerr'.
    kcordict : dict
        K-correction dictionary with filter definitions.
    survey : str
        Survey name key into kcordict.
    colorcut : float, optional
        Not currently used. Default is 0.1.

    Returns
    -------
    scale : float
        Log of inverse scale factor to apply to spectrum.
    colordiffs : array or None
        Color differences (phot - spec) between adjacent filters.
    """

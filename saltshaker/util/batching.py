"""
Data batching utilities for JAX-based training.

This module provides functions to batch training data for efficient use with
JAX's vmap (vectorized map) operations. Data is grouped by size and zero-padded
to enable vectorized computation across batches.

Functions
---------
optimizepaddingsizes
    Determine optimal padding sizes to minimize memory usage.
batchdatabysize
    Divide data into batches grouped by size for vmap.
walkargumenttree
    Helper to determine which arguments should be mapped.
batchedmodelfunctions
    Wrap a function to operate over batched data with vmap.
"""
...
def optimizepaddingsizes(numbatches, datasizes):
    ...
    def parstobins(pars, largest):
        ...
        pars = np.abs(pars)
        pars = np.concatenate([[0], pars, [1.0]])
    ...
    def loss(pars):  #    binlocs=[14,60]
        ...
        spacerequired = stats.binned_statistic(
            datasizes, datasizes, statistic="count", bins=bins
        ).statistic * np.floor(bins[1:])
    ...
    result = optimize.minimize(
        loss, [0] * ndim, method="Nelder-Mead", options={"initial_simplex": vertices}
    )
    pars = result.x
    bins = parstobins(pars, max(datasizes))
    padsizes = stats.binned_statistic(
        datasizes,
        datasizes,
        statistic=lambda x: x.max() if x.size > 0 else 0,
        bins=bins,
    ).statistic
    ...
    def repackforvmap(data):
        ...
        for j, varname in enumerate(data[0].__slots__):
            ...
            if isinstance(vals[0], sparse.BCOO):
                yield sparse.bcoo_concatenate(
                    [x.reshape((1, *x.shape)).update_layout(n_batch=1) for x in vals],
                    dimension=0,
                )
            else:
...
def walkargumenttree(x, targetsize, ncalls=0):
    ...
    if isinstance(x, dict):
        return {
            key: walkargumenttree(val, targetsize, ncalls + 1) for key, val in x.items()
        }
    ...
    def vectorized(pars, *batchedargs, **batchedkwargs):
        ...
        if len(batchedargs) == 0:
            ...
            batchedargs = list(
                zip(
                    *[
                        x if hasattr(x, "__len__") else [x] * len(batcheddata)
                        for x in batchedargs
                    ]
                )
            )
        ...
        if (batchedkwargs) == {}:
            ...
            batchedkwargs = [
                {
                    y: x[i] if hasattr(x, "__len__") else x
                    for y, x in batchedkwargs.items()
                }
                for i in range(len(batcheddata))
            ]
        for batch, indexdict, args, kwargs in zip(
            batcheddata, batchedindexed, batchedargs, batchedkwargs
        ):
                ...
                if not (x in dtype.__ismapped__) and not x == "uniqueid":
                    try:
                        if y.shape[0] == targetsize:
                            log.critical(
                                f"Possible conflict in index mapping, ensure {x} is declared in datamodels as a mapped argument"
                            )
                    except:
                        pass

            inaxes = (
                [(0 if (x in dtype.__ismapped__) else None) for x in dtype.__slots__],
                newpars.mappingaxes,
                *walkargumenttree(newargs, targetsize),
            )
            mapped = jax.vmap(funpacked, in_axes=inaxes)(

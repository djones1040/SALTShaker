"""
Data reading utilities for SALT3 training.

This module provides classes and functions for loading supernova photometry
and spectroscopy data in SNANA format, as well as K-correction files.
...
SALTtraininglightcurve
    Container for a single-filter light curve.
SALTtrainingspectrum
    Container for a single spectrum.
SALTtrainingSN
    Container for all data associated with one supernova.

Functions
---------
rdAllData
    Read all training data from SN list files.
rdkcor
    Read K-correction files for all surveys.
"""
...
class SALTtrainingdata(metaclass=abc.ABCMeta):
    """
    Abstract base class for SALT training data containers.

    Provides a common interface for clipping data based on boolean arrays.
    Subclasses must define ``__listdatakeys__`` listing attributes to clip.
    """
...
class SALTtraininglightcurve(SALTtrainingdata):
    ...
    tpk_guess : float
        Estimated time of B-band maximum (MJD).
    flt : str
        Filter name.
    sn : SuperNova
        SNANA SuperNova object containing the photometry.
    ...
    tobs : ndarray
        Observer-frame time relative to peak (days).
    phase : ndarray
        Rest-frame phase relative to peak (days).
    fluxcal : ndarray
...
class SALTtrainingspectrum(SALTtrainingdata):
    ...
    ----------
    snanaspec : dict
        SNANA spectrum dictionary with keys SPECTRUM_MJD, FLAM, FLAMERR, etc.
    z : float
        Heliocentric redshift.
    tpk_guess : float
        Estimated time of B-band maximum (MJD).
    n_specrecal : int, optional
        Number of spectral recalibration parameters.
    binspecres : float, optional
        If provided, bin spectrum to this rest-frame resolution (Angstroms).

    Attributes
    ----------
    wavelength : ndarray
        Observer-frame wavelengths (Angstroms).
    restwavelength : ndarray
        Rest-frame wavelengths (Angstroms).
    flux : ndarray
        Flux values (erg/s/cm^2/A).
    fluxerr : ndarray
        Flux uncertainties.
    mjd : float
        MJD of observation.
    ...
    phase : float
        Rest-frame phase relative to peak (days).
    n_specrecal : int
        Number of recalibration parameters for this spectrum.
    """
    ...
    def __init__(self, snanaspec, z, tpk_guess, n_specrecal=None, binspecres=None):
        ...
        if ("DQ" in snanaspec and (snanaspec["DQ"] == 1).sum() == 0) or np.all(
            np.isnan(self.flux)
        ):
        ...
        if binspecres is not None:
            ...
            def weighted_err(indices):
                ...
                average = np.average(flux[indices] / fluxmax, weights=weights[indices])
                variance = np.average(
                    (flux[indices] / fluxmax - average) ** 2, weights=weights[indices]
                )
            ...
            wavebins = np.linspace(
                np.min(wavelength),
                np.max(wavelength),
                int((np.max(wavelength) - np.min(wavelength)) / (binspecres * (1 + z))),
            )
            binned_flux = ss.binned_statistic(
                wavelength, range(len(flux)), bins=wavebins, statistic=weighted_avg
            ).statistic
            binned_fluxerr = ss.binned_statistic(
                wavelength, range(len(flux)), bins=wavebins, statistic=weighted_err
            ).statistic

            iGood = binned_flux == binned_flux

            self.flux = binned_flux[iGood]
            self.wavelength = (wavebins[1:][iGood] + wavebins[:-1][iGood]) / 2.0
...
class SALTtrainingSN:
    """
    Container for all training data associated with one supernova.

    Parameters
    ----------
    sn : SuperNova
        SNANA SuperNova object.
    estimate_tpk : bool, optional
        If True, estimate peak MJD using Bazin function fit.
    snpar : Table, optional
        Table of SN parameters including FITPROB for cuts.
    pkmjddict : dict, optional
        Dictionary mapping SNID to peak MJD.
    n_specrecal : Table, optional
        Table specifying recalibration parameters per spectrum.
    binspecres : float, optional
        Spectral binning resolution in rest-frame Angstroms.
    ...
    survey : str
        Survey name (used for K-correction lookup).
    zHelio : float
        Heliocentric redshift.
    MWEBV : float
        Milky Way E(B-V) reddening.
    tpk_guess : float
        Estimated time of B-band maximum (MJD).
    salt2fitprob : float
        SALT2 fit probability (for quality cuts).
    photdata : dict
        Dictionary of SALTtraininglightcurve objects keyed by filter.
    specdata : dict
        Dictionary of SALTtrainingspectrum objects keyed by index.
    """
    ...
    def __init__(
        ...
        if "SURVEY" in sn.__dict__.keys():
            ...
            raise SNDataReadError(
                "SN %s has no SURVEY key, which is needed to find the filter transmission curves"
                % sn.SNID[0]
            )
        if not "REDSHIFT_HELIO" in sn.__dict__.keys():
            raise SNDataReadError(
                "SN %s has no heliocentric redshift information in the header" % sn.SNID
            )

        if "PEAKMJD" in sn.__dict__.keys():
            sn.SEARCH_PEAKMJD = sn.PEAKMJD
        # FITS vs. ASCII format issue in the parser
        if isinstance(sn.REDSHIFT_HELIO, str):
            self.zHelio = float(sn.REDSHIFT_HELIO.split("+-")[0])
        else:
            self.zHelio = sn.REDSHIFT_HELIO
        ...
        if estimate_tpk:
            if "B" in sn.FLT:
                tpk, tpkmsg = estimate_tpk_bazin(
                    sn.MJD[sn.FLT == "B"],
                    sn.FLUXCAL[sn.FLT == "B"],
                    sn.FLUXCALERR[sn.FLT == "B"],
                    max_nfev=100000,
                    t0=sn.SEARCH_PEAKMJD,
                )
            elif "g" in sn.FLT:
                tpk, tpkmsg = estimate_tpk_bazin(
                    sn.MJD[sn.FLT == "g"],
                    sn.FLUXCAL[sn.FLT == "g"],
                    sn.FLUXCALERR[sn.FLT == "g"],
                    max_nfev=100000,
                    t0=sn.SEARCH_PEAKMJD,
                )
            elif "c" in sn.FLT:
        ...
        if "MWEBV" in sn.__dict__.keys():
            try:
                self.MWEBV = float(sn.MWEBV.split()[0])
            except:
                self.MWEBV = float(sn.MWEBV)
        elif "RA" in sn.__dict__.keys() and "DEC" in sn.__dict__.keys():
            log.warning(
                "determining MW E(B-V) from IRSA for SN %s using RA/Dec in file"
                % sn.SNID
            )
            ...
            raise SNDataReadError(
                "Could not determine E(B-V) from files.   Set MWEBV keyword in input file header for SN %s"
                % sn.SNID
            )
        ...
        self.photdata = {
            flt: SALTtraininglightcurve(
                self.zHelio, tpk_guess=self.tpk_guess, flt=flt, sn=sn
            )
            for flt in np.unique(sn.FLT)
        }
        try:
            assert len(self.photdata) > 0
            ...
            for speccount, k in enumerate(sn.SPECTRA):
                ...
                try:
                    ...
                    if hasattr(e, "message"):
                        log.warning(
                            f"{e.message}, skipping spectrum {k} for SN {self.snid}"
                        )
                    else:
                        log.warning(
                            f"DataReadError, skipping spectrum {k} for SN {self.snid}"
                        )
...
def rdkcor(surveylist, options):
    ...
    Parameters
    ----------
    surveylist : list of str
        List of survey names to load.
    options : argparse.Namespace
        Configuration options containing kcorfile paths per survey.
    ...
    kcordict : dict
        Nested dictionary with structure::
            ...
            kcordict[survey][filter] = {
                'filtwave': ndarray,      # filter wavelengths
                'filttrans': ndarray,     # filter transmission
                'lambdaeff': float,       # effective wavelength
                'magsys': str,            # 'AB', 'Vega', or 'BD17'
                'zpoff': float,           # zeropoint offset
                ...
            }

    Notes
    -----
    Also applies calibration shifts if options.calibrationshiftfile is set.
    """
    kcordict = {}
    for survey in surveylist:
        ...
        if not os.path.exists(kcorfile):
            log.info(
                "kcor file %s does not exist.   Checking %s/kcor"
                % (kcorfile, data_rootdir)
            )
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                raise RuntimeError(
                    "kcor file format is non-standard for kcor file %s" % kcorfile
                )
        ...
        for subsurvey in subsurveys:
            ...
            for filt in zpoff["Filter Name"]:
                ...
                kcordict[kcorkey][internalfiltname]["filtwave"] = np.array(
                    filtertrans["wavelength (A)"]
                )
                kcordict[kcorkey][internalfiltname][
                    "fullname"
                ] = filt  # .split('/')[0].replace(' ','')
                kcordict[kcorkey][internalfiltname]["filttrans"] = np.array(
                    filtertrans[filt]
                )

                lambdaeff = np.sum(
                    kcordict[kcorkey][internalfiltname]["filtwave"] * filtertrans[filt]
                ) / np.sum(filtertrans[filt])
                kcordict[kcorkey][internalfiltname]["lambdaeff"] = lambdaeff
                kcordict[kcorkey][internalfiltname]["magsys"] = zpoff["Primary Name"][
                    zpoff["Filter Name"] == filt
                ][0]
                kcordict[kcorkey][internalfiltname]["primarymag"] = (
                    zpoff["Primary Mag"][zpoff["Filter Name"] == filt][0]
                    - zpoff["ZPoff(SNpot)"][zpoff["Filter Name"] == filt][0]
                )
                        ...
                        for survey_forfilt in kcordict.keys():
                            if filter in kcordict[survey_forfilt].keys():
                                if shifttype == "MAGSHIFT":
                                    ...
                                    kcordict[survey_forfilt][filter][
                                        "primarymag"
                                    ] += shift
                                    has_filter = True
                                elif (
                                    shifttype == "LAMSHIFT" or shifttype == "WAVESHIFT"
                                ):
                                    kcordict[survey_forfilt][filter][
                                        "filtwave"
                                    ] += shift
                                    ...
                                    raise ValueError(
                                        f"Invalid calibration shift: {shifttype}"
                                    )
                            if not has_filter:
                                raise ValueError(
                                    f"could not find filter {filter} in any kcor files"
                                )
...
def rdAllData(
    ...
    ----------
    snlists : str
        Comma-separated list of paths to SN list files.
    estimate_tpk : bool
        If True, estimate peak MJD using Bazin function fit.
    dospec : bool, optional
        If True, read spectroscopic data. Default is False.
    peakmjdlist : str, optional
        Path to file with peak MJD values (columns: SNID, MJD).
    waverange : list of float, optional
        Wavelength range [min, max] in Angstroms. Default is [2000, 9200].
    binspecres : float, optional
        If provided, bin spectra to this rest-frame resolution.
    snparlist : str, optional
        Path to file with SN parameters (for FITPROB cuts).
    specrecallist : str, optional
        Path to file specifying spectral recalibration parameters.
    maxsn : int, optional
        Maximum number of SNe to read (for debugging).
    ...
    for snlist in snlists.split(","):
        ...
        if not os.path.exists(snlist):
            log.info(
                "SN list file %s does not exist.       Checking %s/trainingdata/%s"
                % (snlist, data_rootdir, snlist)
            )
    ...
    if maxsn is not None:
        maxcount = nsnperlist * maxsn / nsnperlist.sum()
    else:
        maxcount = [np.inf] * len(snlists.split(","))
    ...
    def processsupernovaobject(outputdict, sn, maxnum, n_specrecal):
        ...
        if "FLT" not in sn.__dict__.keys() and "BAND" in sn.__dict__.keys():
            sn.FLT = sn.BAND

        sn.SNID = str(sn.SNID)

        if sn.SNID in datadict:
            duplicatesurvey = datadict[sn.SNID].survey
        elif sn.SNID in outputdict:
            duplicatesurvey = outputdict[sn.SNID].survey
        else:
            duplicatesurvey = None
        if not duplicatesurvey is None:
            log.warning(
                f"SNID {sn.SNID} is a duplicate! Keeping version from survey {duplicatesurvey}, discarding version from survey {sn.SURVEY}"
            )
            return False
        ...
        try:
            for f in snfiles:
                ...
                if f.lower().endswith(".fits") or f.lower().endswith(".fits.gz"):
                    ...
                    if (
                        f.lower().endswith(".fits")
                        and not os.path.exists(f)
                        and os.path.exists("{}.gz".format(f))
                    ):
                        f = "{}.gz".format(f)
                    # get list of SNIDs
                    ...
                    for snid in snidlist:
                        ...
                        sn = snana.SuperNova(
                            snid=snid,
                            headfitsfile=f,
                            photfitsfile=f.replace("_HEAD.FITS", "_PHOT.FITS"),
                            specfitsfile=specfitsfile,
                            readspec=dospec,
                        )
                        if (
                            "SUBSURVEY" in sn.__dict__.keys()
                            and not (
                                len(np.unique(sn.SUBSURVEY)) == 1
                                and survey.strip() == np.unique(sn.SUBSURVEY)[0].strip()
                            )
                        ...
                        if specrecallist:
                            ...
                            n_specrecal = None
                        skipcount += not processsupernovaobject(
                            snreadinfromlist, sn, maxct, n_specrecal
                        )
                else:
                    if "/" not in f:

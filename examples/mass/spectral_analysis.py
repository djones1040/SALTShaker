...
import copy
import extinction as ex #from specutils 
...
def measure_velocity(wavelength, flux, wave1, wave2, vexp=.001, clip=True, rest_wave=6355., varflux=None, plot=False, error=False):
    ...
    if len(si_min_index[0]) > 0. and (wavelength[-1] > wave2):
        ...
        if plot:
            ...
            zoom = (wavelength>wave1-500) & (wavelength < wave2+500)
            # zoom_old_sig = (old_wave[sigclip_region]>5500) & (old_wave[sigclip_region] < 6500)
            zoom_old = (old_wave>wave1-500) & (old_wave < wave2+500)
            norm = 1./np.amax(flux)
        ...
        si_min_wave = np.nan
        sigma = np.nan
    #if error: import pdb; pdb.set_trace()
    if error:

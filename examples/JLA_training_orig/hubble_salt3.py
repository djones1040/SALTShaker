...
def main():
    ...
    ax1.errorbar(fr2.zCMB,fr2.mures-np.median(fr2.mures[fr2.zCMB > 0.01]),yerr=fr2.muerr,fmt='o',color=colordict['SALT2'],alpha=0.1)
    ax1.errorbar(fr3.zCMB,fr3.mures-np.median(fr3.mures[fr3.zCMB > 0.01]),yerr=fr3.muerr,fmt='D',color=colordict['SALT3'],alpha=0.1)
    ax1.errorbar((zbins[1:]+zbins[:-1])/2.,salt2mubins,yerr=salt2muerrbins,fmt='o-',color=colordict['SALT2'])
    ax1.errorbar((zbins[1:]+zbins[:-1])/2.,salt3mubins,yerr=salt3muerrbins,fmt='D-',color=colordict['SALT3'])
    ax1.set_xscale('log')
    ...
    ax2.hist(fr2.mures[fr2.zCMB > 0.01]-np.median(fr2.mures[fr2.zCMB > 0.01]),bins=muresbins,
             orientation='horizontal',color=colordict['SALT2'],alpha=0.5)
    ax2.hist(fr3.mures[fr3.zCMB > 0.01]-np.median(fr3.mures[fr3.zCMB > 0.01]),bins=muresbins,
             orientation='horizontal',color=colordict['SALT3'],alpha=0.5)
...
$med. \chi^2_{{\nu}}$ = {med_chi2:.2f}""",
             ha='left',va='top',color=colordict['SALT2'],transform=ax1.transAxes,bbox={'facecolor':'1.0','edgecolor':'1.0','alpha':0.5})
...
$med. \chi^2_{{\nu}}$ = {med_chi3:.2f}""",
             ha='left',va='top',color=colordict['SALT3'],transform=ax1.transAxes,bbox={'facecolor':'1.0','edgecolor':'1.0','alpha':0.5})

    ax2.text(0.97,0.97,f"""SALT2 RMS = {np.std(fr2.mures[fr2.zCMB > 0.015]):.3f}
{iOut2*len(fr2.CID):.0f} outliers ({iOut2*100:.1f}%)""",transform=ax2.transAxes,ha='right',va='top',color=colordict['SALT2'])
